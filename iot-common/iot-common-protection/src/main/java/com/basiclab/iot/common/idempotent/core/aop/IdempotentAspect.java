package com.basiclab.iot.common.idempotent.core.aop;
...
public class IdempotentAspect {
    ...
    private final IdempotentRedisDAO idempotentRedisMapper;

    public IdempotentAspect(List<IdempotentKeyResolver> keyResolvers, IdempotentRedisDAO idempotentRedisMapper) {
        this.keyResolvers = CollectionUtils.convertMap(keyResolvers, IdempotentKeyResolver::getClass);
        this.idempotentRedisMapper = idempotentRedisMapper;
    }
    ...
    public Object aroundPointCut(ProceedingJoinPoint joinPoint, Idempotent idempotent) throws Throwable {
        ...
        // 1. 锁定 Key
        boolean success = idempotentRedisMapper.setIfAbsent(key, idempotent.timeout(), idempotent.timeUnit());
        // 锁定失败，抛出异常
        if (!success) {
            log.info("[aroundPointCut][方法({}) 参数({}) 存在重复请求]", joinPoint.getSignature().toString(), joinPoint.getArgs());
            throw new ServiceException(GlobalErrorStatus.REPEATED_REQUESTS.getCode(), idempotent.message());
        }
        ...
        try {
            ...
            if (idempotent.deleteKeyWhenException()) {
                idempotentRedisMapper.delete(key);
            }
            throw throwable;
        }
    }
