#!/usr/bin/env bash
# CentOS/RHEL ARM64：按 EL 大版本（7/8/9）交叉打包
#   bash COMPILE/platforms/centos-arm/build.sh --el9
#   bash COMPILE/platforms/centos-arm/build.sh --el8
#   bash COMPILE/platforms/centos-arm/build.sh --el7
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
COMPILE_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
REPO_ROOT="$(cd "${COMPILE_ROOT}/.." && pwd)"
# shellcheck source=../centos/el_common.sh
source "${COMPILE_ROOT}/platforms/centos/el_common.sh"

EL_ARG="${EL_RELEASE:-9}"
MODE="docker"
DO_RPM=1
DOCKER_PLATFORM="${COMPILE_DOCKER_PLATFORM:-linux/arm64}"
...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      ...
      echo "  COMPILE_DOCKER_PLATFORM  默认 linux/arm64"
      echo "  COMPILE_OUT              默认 COMPILE/dist/centos-arm-el{7|8|9}"
      exit 0
      ;;
    *)
      echo "[COMPILE/centos-arm] 未知参数: $arg" >&2
      exit 1
      ;;
  esac
done
...
VENV_DIR="${COMPILE_ROOT}/.venv-build-centos-arm-${EL_OUT_SUFFIX}"
...
require_arm_host_for_local() {
  local arch
  arch="$(uname -m)"
  case "$arch" in
    aarch64|arm64) return 0 ;;
  esac
  echo "[COMPILE/centos-arm-${EL_OUT_SUFFIX}] --local 仅支持 aarch64/arm64（当前: ${arch}）" >&2
  echo "请改用: bash COMPILE/build.sh centos-arm-el${EL_RELEASE}" >&2
  exit 1
}
...
build_local() {
  ...
  if ! py="$(pick_python)"; then
    echo "[COMPILE/centos-arm-${EL_OUT_SUFFIX}] 需要 python3（建议 >= 3.8）" >&2
    exit 1
  fi
  ...
  if [ "${SKIP_UI_BUILD:-0}" = "1" ] && [ -f "${REPO_ROOT}/PANEL/ui/dist/index.html" ]; then
    ...
    if ! command -v npm >/dev/null 2>&1; then
      echo "[COMPILE/centos-arm-${EL_OUT_SUFFIX}] 本地构建前端需要 npm" >&2
      exit 1
    fi
    log "构建前端 ui/dist"
  ...
  WORK_DIR="${COMPILE_ROOT}/work/centos-arm-${EL_OUT_SUFFIX}"
  rm -rf "$WORK_DIR"
  mkdir -p "$WORK_DIR"

  log "PyInstaller 生成 Linux ARM64 二进制 (el${EL_RELEASE})"
  pyinstaller \
    --clean \
    --noconfirm \
    --distpath "$OUT_DIR" \
  ...
  if [ ! -f "${OUT_DIR}/easyaiot-panel" ]; then
    echo "[COMPILE/centos-arm-${EL_OUT_SUFFIX}] 未生成 easyaiot-panel" >&2
    exit 1
  fi
...
export INSTALL_SCRIPT="${INSTALL_SCRIPT:-.scripts/docker/install_linux_centos_arm.sh}"
exec "${HERE}/easyaiot-panel"
EOF
  chmod +x "${OUT_DIR}/run.sh"

  if [ "$EL_RELEASE" = "9" ] && [ "${COMPILE_OUT_SKIP_LEGACY_LINK:-0}" != "1" ]; then
    local legacy="${COMPILE_ROOT}/dist/centos-arm"
    mkdir -p "$legacy"
  ...
  if [ "$DO_RPM" -eq 1 ]; then
    PANEL_ARCH=aarch64 EL_RELEASE="$EL_RELEASE" PANEL_DIST_TAG="$PANEL_DIST_TAG" \
...
build_docker() {
  if ! command -v docker >/dev/null 2>&1; then
    echo "[COMPILE/centos-arm-${EL_OUT_SUFFIX}] 需要 Docker（linux/arm64 交叉构建）" >&2
    exit 1
  fi
  ...
  mkdir -p "$OUT_DIR"
  CTX_DIR="${COMPILE_ROOT}/work/centos-arm-${EL_OUT_SUFFIX}-docker-context"
  rm -rf "${CTX_DIR}"
  ...
  if [ "$DO_RPM" -eq 1 ]; then
    log "Docker 交叉构建 CentOS ARM ${DIST_TAG} + RPM (platform=${DOCKER_PLATFORM} base=${BASE_IMAGE} version=${PANEL_VERSION_ARG})"
  else
    log "Docker 交叉构建 CentOS ARM ${DIST_TAG} (platform=${DOCKER_PLATFORM} base=${BASE_IMAGE})"
  fi
