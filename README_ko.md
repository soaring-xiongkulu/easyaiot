...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>원자 모드: 연산 박스에는 실행기만</strong>: 엣지 박스 / 클러스터 워커는 <strong>RUNTIME 바이너리만 설치</strong> — 로컬 VIDEO / WEB / DEVICE 없음. 설치 가벼움, 장애면 축소, 확장은 「플랫폼 재설치」가 아니라 「연산 추가」. 경보·하트비트는 센터 VIDEO로 집약; 센터가 실시간 / 캡처 / 순찰(<code>executor=cpp</code>)을 동일 실행 프로세스에 하달하고, 실시간은 기본으로 박스 검출 스트림을 센터 SRS에 푸시. 원자 = 로컬 업무면 없음(미푸시가 아님). 원클릭: <code>VIDEO_BASE_URL=http://&lt;센터&gt;:6000 bash .scripts/docker/install_linux.sh runtime</code>; 또는 WEB 일괄 배포</li>
  ...
  <li><strong>RUNTIME 원자 엣지 연산</strong>: 광역 배치·연산 박스 확장용 — 노드는 RUNTIME 실행기만 설치, 경보는 MQTT로 iot-sink에, 하트비트는 VIDEO로; 지점마다 전체 업무 스택을 쌓을 필요 없음. 기존 EDGE 연합 엣지 스위트는 제거되었으며 이 경로로 통일</li>
  ...
  <li><strong>알고리즘 태스크 관리</strong>: 실시간·스냅샷·순찰 유형의 알고리즘 태스크 생성 및 관리를 지원하며, 각 태스크는 프레임 추출기와 정렬기를 유연하게 바인딩하여 정밀한 비디오 프레임 추출 및 결과 정렬을 실현합니다.
    <ul style="margin: 5px 0; padding-left: 20px;">
      ...
      <li><strong>순찰 알고리즘 태스크</strong>: 다채널 라운드로빈 / 연결 풀; 기본으로도 <code>executor=cpp</code>(RUNTIME PatrolScheduler); 노드는 원자 모드로 실행기만 설치해 수용 가능</li>
    </ul>
  </li>
...
<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
EasyAIoT는 WEB, APP, DEVICE, NODE, VIDEO, RTC, AI, RUNTIME, VISUALIZE, TRANSFORM, PANEL, SITE 등 핵심 모듈로 구성되며, COMPILE 다중 플랫폼 패키징·납품 능력을 함께 제공합니다(EDGE 제거, 엣지는 RUNTIME 원자 모드):
</p>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>노드 역할</strong>: compute / media / hybrid 역할로 크로스 노드 스케줄링·탄력 확장; 연산 노드는 <strong>RUNTIME 원자 모드</strong>(실행기만)로 센터가 하달한 실시간 / 캡처 / 순찰·고성능 전달을 수용</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
    ...
    <li><strong>원자 모드</strong>: 연산 박스/워커는 실행기만 설치 — VIDEO/WEB/DEVICE 없음; 센터 오케스트레이션·노드 실행 — 전체 플랫폼이 아니라 연산 추가처럼 확장</li>
    <li><strong>원클릭 배포</strong>: WEB 「업무 런타임 배포」 또는 설치 스크립트 일괄 배포; 센터 VIDEO 설치 시 로컬 실행기 자동 마운트</li>
  </ul>
</td>
</tr>
<tr>
<td style="padding: 15px; border: 1px solid #e0e0e0; vertical-align: top;"><strong>RUNTIME 원자 엣지</strong></td>
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>EDGE 모듈 제거</strong>: 엣지 연산은 <strong>RUNTIME 원자 모드</strong>(실행기만 설치)로 통일</li>
    <li><strong>이벤트 평면 MQTT</strong>: 경보/캡처/후처리는 EMQX 알고리즘 버스로, <strong>iot-sink</strong>가 저장·아카이브·알림 enrichment</li>
    <li><strong>관리 평면 HTTP 하트비트</strong>: 작업 생존은 여전히 VIDEO로, 시작/중지·작업 테이블은 VIDEO 관리</li>
    <li><strong>NFS 미디어 루트</strong>: 경보 이미지·SRS 녹화는 <strong>NFS 공유 미디어 루트</strong>(<code>EASYAIOT_MEDIA_ROOT</code>, 기본 <code>/mnt/easyaiot-media</code>; sudo 없으면 <code>$HOME/easyaiot/media</code>)에 통일; MQTT는 경로만 전달, <strong>iot-sink</strong>가 디스크 읽기 후 MinIO 아카이브</li>
  </ul>
</td>
</tr>
<tr>
