...
- **Disk**: Recommended 200GB or more available space
...
For detailed Windows deployment instructions, including environment preparation, middleware deployment, service startup, and troubleshooting, please refer to the dedicated [Windows Deployment Guide](平台Windows部署文档.md).

The Windows deployment guide provides comprehensive step-by-step instructions for:
...
This guide is specifically tailored for Windows 10/11 systems and covers local deployment scenarios.
...
**⚠️ Dangerous Operation**: Delete all containers, images, and data volumes
...
For Windows deployment, please refer to the dedicated [Windows Deployment Guide](平台Windows部署文档.md) for comprehensive step-by-step instructions. The guide covers local deployment scenarios on Windows 10/11 systems, including:
...
**Note**: The Windows deployment guide focuses on local deployment. For Docker-based deployment on Windows, you can use the `install_win.ps1` script, but detailed instructions are available in the Windows deployment guide.
