...
- ✅ 智能合并短段落
- ✅ 清理多余换行和空格
- ✅ 自动识别标题并加粗（包括带emoji图标的标题和Markdown加粗格式）
- ✅ 统一字体为微软雅黑
- ✅ 优化段落间距和行距
- ✅ 项目地址自动识别并设置为红色小字体
...
2. **标题识别**: 
   - 自动识别常见标题（如：引言、项目概述、技术架构等）
   - 自动识别带emoji图标的标题（如：💡 引言、📋 项目概述等）
   - 自动识别Markdown加粗格式的标题（如：**设备接入与注册**）
   - 标题独立成行，不会被合并到其他段落
   - 标题格式：18pt，微软雅黑，加粗，蓝色（#0066CC）
3. **项目地址处理**:
   - 自动识别包含"项目地址：https://..."的文本
   - 项目地址格式：11pt，红色（#DC143C），不影响正文观感
   - "项目地址："前缀：12pt，正常颜色
4. **字体设置**: 
   - 正文: 14pt，微软雅黑（公众号常用，更美观）
   - 标题: 18pt，微软雅黑，加粗，蓝色（更醒目美观）
   - 项目地址: 11pt，红色（更小字体，不影响正文）
5. **间距优化**:
   - 段落间距: 6pt（更紧凑美观）
   - 行距: 1.5倍（更美观）
   - 标题前后间距: 12pt/10pt（标题更突出）
...
- `Pt(18)`: 标题字号（当前设置为18pt）
- `RGBColor(0, 102, 204)`: 标题颜色（蓝色 #0066CC）
- `RGBColor(220, 20, 60)`: 项目地址颜色（红色 #DC143C）
- `Pt(11)`: 项目地址字号（当前设置为11pt，不影响正文观感）
- `Pt(6)`: 段落间距（当前设置为6pt）
- `1.5`: 行距倍数（当前设置为1.5倍）
- `is_title()`: 标题识别规则（支持emoji图标标题、Markdown加粗格式**文本**，可添加更多标题关键词）
- `is_project_url()`: 项目地址识别规则
- `should_merge()`: 段落合并规则
