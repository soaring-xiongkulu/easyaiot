<template>
  ...
  >
    <div class="face-workbench">
      ...
      <div class="workbench-toolbar">
        ...
        <span class="workbench-toolbar__hint" v-if="status === 'pending'">
          AI 检测到但未匹配人脸库的人员会出现在这里，确认标注后即可批量入库
        </span>
      </div>
            ...
            >
              ...
              <div class="target-card__cover" @click="openEnroll(item)">
                ...
                <div class="target-card__overlay">
                  <button class="overlay-btn" title="修正标注并入库" @click.stop="openEnroll(item)">
                    <AuditOutlined />
                  </button>
                  <button
    ...
    <FaceEnrollDrawer @register="registerEnrollDrawer" @success="onEnrollSuccess" />
    <FaceBatchEnrollModal @register="registerBatchModal" @success="onBatchEnrollSuccess" />
  </BasicDrawer>
</template>

<script lang="ts" setup>
/**
 * 人脸待入库工作台：集中管理算法任务中「检测到但未匹配人脸库」的目标。
 * 支持批量入库 / 忽略 / 恢复 / 删除，单条修正标注后两步入库。
 */
...
import { resolveFaceImageDisplayUrl } from '@/api/device/face_library';
import {
...
import FaceEnrollDrawer from './FaceEnrollDrawer.vue';
import FaceBatchEnrollModal from './FaceBatchEnrollModal.vue';

const Input = AInput;

defineOptions({ name: 'FacePendingWorkbench' });
...
const emptyText = computed(() => {
  if (status.value === 'pending') {
    return '暂无待入库目标。算法任务开启人脸检测并匹配人脸库后，未匹配到的人员会自动出现在这里';
  }
...
async function loadRecords() {
  ...
  try {
    const res = await listPendingRecords('face', {
      status: status.value,
      page: page.value,
...
async function handleDiscardOne(item: PendingEnrollRecord) {
  try {
    await batchDiscardPendingRecords('face', [item.id]);
...
async function handleRestoreOne(item: PendingEnrollRecord) {
  try {
    await batchRestorePendingRecords('face', [item.id]);
...
async function handleDeleteOne(item: PendingEnrollRecord) {
  try {
    await batchDeletePendingRecords('face', [item.id]);
...
async function handleBatchDiscard() {
  ...
  try {
    const res = await batchDiscardPendingRecords('face', [...checkedIds.value]);
...
async function handleBatchRestore() {
  if (!checkedIds.value.length) return;
  try {
    const res = await batchRestorePendingRecords('face', [...checkedIds.value]);
...
async function handleBatchDelete() {
  if (!checkedIds.value.length) return;
  try {
    const res = await batchDeletePendingRecords('face', [...checkedIds.value]);
