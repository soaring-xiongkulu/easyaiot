...
public class IotGatewayProperties {
    ...
    public static class TopicProperties {
        ...
        /**
         * 是否启用 Topic 订阅
         * true: 启用，会根据 topicList 配置决定订阅哪些 topic
         * false: 禁用，不订阅任何 topic
         */
         ...
         * Topic 列表
         * 为空：默认订阅所有 topic
         * 不为空：只订阅列表中的 topic（使用 IotDeviceTopicEnum 的枚举名称，如 PROPERTY_UPSTREAM_REPORT）
         */
         ...
         * @return true: 应该处理，false: 不应该处理
         */
        public boolean shouldProcess(com.basiclab.iot.sink.enums.IotDeviceTopicEnum topicEnum) {
            // 如果未启用，不处理任何 topic
            if (enabled == null || !enabled) {
                return false;
            }

            // 如果 topicList 为空，默认处理所有 topic
            if (topicList == null || topicList.isEmpty()) {
                return true;
            }

            // 如果 topicList 不为空，只处理列表中的 topic
            return topicList.contains(topicEnum.name());
        }
    }
