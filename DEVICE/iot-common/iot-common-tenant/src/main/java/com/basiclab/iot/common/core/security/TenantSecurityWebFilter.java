    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (user != null) {
            ...
            if (tenantId == null) {
                ...
                ServletUtils.writeJSON(response, CommonResult.error(GlobalErrorStatus.FORBIDDEN.getCode(),
                        "您无权访问该租户的数据"));
                return;
            }
        }
        ...
        if (!isIgnoreUrl(request)) {
            ...
            if (tenantId == null) {
                ...
                ServletUtils.writeJSON(response, CommonResult.error(GlobalErrorStatus.BAD_REQUEST.getCode(),
                        "请求的租户标识未传递，请进行排查"));
                return;
            }
            // 3. 校验租户是合法，例如说被禁用、到期
