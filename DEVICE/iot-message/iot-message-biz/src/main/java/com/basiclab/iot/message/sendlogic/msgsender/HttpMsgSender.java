...
public class HttpMsgSender implements IMsgSender {
    ...
    public HttpSendResult send(String msgId) {
        log.info("http发送开始 params is:" + msgId);
        return sendUseOkHttp(msgId);
    }
    ...
    public HttpSendResult sendUseHutool(String msgId) {
        ...
        try {
            ...
            if (httpMsg.isHttpUseProxy()) {
                httpRequest.setProxy(getProxy(configMap));
            }
    ...
    public HttpSendResult sendUseOkHttp(String msgId) {
        ...
        try {
            ...
            if (!"GET".equals(httpMsg.getMethod()) && httpMsg.getParamMap() != null && !httpMsg.getParamMap().isEmpty()) {
            ...
            } else if (!"GET".equals(httpMsg.getMethod()) && StringUtils.isNotEmpty(httpMsg.getBody())) {
                String bodyType = httpMsg.getBodyType();
        ...
        if (okHttpClient == null) {
            synchronized (HttpMsgSender.class) {
                if (okHttpClient == null) {
                    ...
                    boolean isHttpUseProxy = (boolean) configMap.get("isHttpUseProxy");
                    if (isHttpUseProxy) {
                        builder.proxy(getProxy(configMap));
                    }
