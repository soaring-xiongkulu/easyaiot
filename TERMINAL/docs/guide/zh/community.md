# 问题反馈与社区

## 问题反馈

遇到 Bug 或有功能建议，欢迎通过以下渠道反馈：

- [GitHub Issues](https://github.com/ys-ll/uniterm/issues)

提交 Issue 时请尽量包含：
- 操作系统和版本
- 终端 版本号
- 问题描述和复现步骤
- 相关截图或日志

## 参与贡献

欢迎提交 Pull Request 参与开发，提交前请阅读[贡献指南](https://github.com/ys-ll/uniterm/blob/main/CONTRIBUTING.md)。

- [GitHub 仓库](https://github.com/ys-ll/uniterm)

## 交流讨论

- [GitHub Discussions](https://github.com/ys-ll/uniterm/discussions)

## 支持项目

如果觉得 终端 好用，欢迎给项目点个 Star ⭐

- [GitHub](https://github.com/ys-ll/uniterm)
- [Gitee](https://gitee.com/ys-l/uniterm)

## 开源协议

终端 基于 [Apache 2.0](https://github.com/ys-ll/uniterm/blob/main/LICENSE) 协议开源。
