...
import com.basiclab.iot.infra.dal.mysql.demo.demo02.Demo02CategoryMapper;
...
public class Demo02CategoryServiceImpl implements Demo02CategoryService {
    ...
    public void deleteDemo02Category(Long id) {
        ...
        if (demo02CategoryMapper.selectCountByParentId(id) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO02_CATEGORY_EXITS_CHILDREN);
        }
        // 删除
    ...
    private void validateDemo02CategoryExists(Long id) {
        if (demo02CategoryMapper.selectById(id) == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO02_CATEGORY_NOT_EXISTS);
        }
    }
    ...
    private void validateParentDemo02Category(Long id, Long parentId) {
        ...
        if (Objects.equals(id, parentId)) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO02_CATEGORY_PARENT_ERROR);
        }
        // 2. 父示例分类不存在
        Demo02CategoryDO parentDemo02Category = demo02CategoryMapper.selectById(parentId);
        if (parentDemo02Category == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO02_CATEGORY_PARENT_NOT_EXITS);
        }
        ...
        for (int i = 0; i < Short.MAX_VALUE; i++) {
            ...
            if (Objects.equals(id, parentId)) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO02_CATEGORY_PARENT_IS_CHILD);
            }
            // 3.2 继续递归下一级父示例分类
            if (parentId == null || Demo02CategoryDO.PARENT_ID_ROOT.equals(parentId)) {
                break;
    ...
    private void validateDemo02CategoryNameUnique(Long id, Long parentId, String name) {
        ...
        if (id == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO02_CATEGORY_NAME_DUPLICATE);
        }
