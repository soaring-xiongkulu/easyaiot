package com.basiclab.iot.infra.service.codegen;
...
import com.basiclab.iot.common.test.core.ut.BaseDbUnitTest;
import com.basiclab.iot.common.test.core.util.AssertUtils;
import com.basiclab.iot.infra.controller.admin.codegen.vo.CodegenCreateListReqVO;
import com.basiclab.iot.infra.controller.admin.codegen.vo.CodegenUpdateReqVO;
import com.basiclab.iot.infra.controller.admin.codegen.vo.column.CodegenColumnSaveReqVO;
import com.basiclab.iot.infra.controller.admin.codegen.vo.table.CodegenTablePageReqVO;
import com.basiclab.iot.infra.controller.admin.codegen.vo.table.DatabaseTableRespVO;
...
import com.basiclab.iot.infra.dal.mysql.codegen.CodegenColumnMapper;
import com.basiclab.iot.infra.dal.mysql.codegen.CodegenTableMapper;
import com.basiclab.iot.infra.enums.ErrorCodeConstants;
import com.basiclab.iot.infra.enums.codegen.CodegenFrontTypeEnum;
import com.basiclab.iot.infra.enums.codegen.CodegenSceneEnum;
import com.basiclab.iot.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.basiclab.iot.infra.framework.codegen.config.CodegenProperties;
import com.basiclab.iot.infra.service.codegen.inner.CodegenBuilder;
import com.basiclab.iot.infra.service.codegen.inner.CodegenEngine;
import com.basiclab.iot.infra.service.db.DatabaseTableService;
...
import static com.basiclab.iot.common.utils.date.LocalDateTimeUtils.buildBetweenTime;
import static com.basiclab.iot.common.utils.date.LocalDateTimeUtils.buildTime;
import static com.basiclab.iot.common.utils.object.ObjectUtils.cloneIgnoreId;
import static com.basiclab.iot.common.test.core.util.AssertUtils.assertPojoEquals;
import static com.basiclab.iot.common.test.core.util.AssertUtils.assertServiceException;
import static com.basiclab.iot.common.test.core.util.RandomUtils.*;
...
public class CodegenServiceImplTest extends BaseDbUnitTest {
    ...
    public void testValidateTableInfo() {
        ...
        assertServiceException(() -> codegenService.validateTableInfo(null),
                ErrorCodeConstants.CODEGEN_IMPORT_TABLE_NULL);
        // 情况二
        TableInfo tableInfo = mock(TableInfo.class);
        assertServiceException(() -> codegenService.validateTableInfo(tableInfo),
                ErrorCodeConstants.CODEGEN_TABLE_INFO_TABLE_COMMENT_IS_NULL);
        // 情况三
        when(tableInfo.getComment()).thenReturn("芋艿");
        assertServiceException(() -> codegenService.validateTableInfo(tableInfo),
                ErrorCodeConstants.CODEGEN_IMPORT_COLUMNS_NULL);
        // 情况四
        TableField field = mock(TableField.class);
        when(field.getName()).thenReturn("name");
        when(tableInfo.getFields()).thenReturn(Collections.singletonList(field));
        AssertUtils.assertServiceException(() -> codegenService.validateTableInfo(tableInfo),
                ErrorCodeConstants.CODEGEN_TABLE_INFO_COLUMN_COMMENT_IS_NULL, field.getName());
    }
    ...
    public void testUpdateCodegen_notExists() {
        ...
        assertServiceException(() -> codegenService.updateCodegen(updateReqVO),
                ErrorCodeConstants.CODEGEN_TABLE_NOT_EXISTS);
    }
    ...
    public void testUpdateCodegen_sub_masterNotExists() {
        ...
        // 调用，并断言
        assertServiceException(() -> codegenService.updateCodegen(updateReqVO),
                ErrorCodeConstants.CODEGEN_MASTER_TABLE_NOT_EXISTS, updateReqVO.getTable().getMasterTableId());
    }
    ...
    public void testUpdateCodegen_sub_columnNotExists() {
        ...
        // 调用，并断言
        assertServiceException(() -> codegenService.updateCodegen(updateReqVO),
                ErrorCodeConstants.CODEGEN_SUB_COLUMN_NOT_EXISTS, updateReqVO.getTable().getSubJoinColumnId());
    }
    ...
    public void testGenerationCodes_columnNotExists() {
        ...
        assertServiceException(() -> codegenService.generationCodes(tableId),
                ErrorCodeConstants.CODEGEN_COLUMN_NOT_EXISTS);
    }
    ...
    public void testGenerationCodes_sub_tableNotExists() {
        ...
        // 调用，并断言
        assertServiceException(() -> codegenService.generationCodes(tableId),
                ErrorCodeConstants.CODEGEN_MASTER_GENERATION_FAIL_NO_SUB_TABLE);
    }
    ...
    public void testGenerationCodes_sub_columnNotExists() {
        ...
        // 调用，并断言
        assertServiceException(() -> codegenService.generationCodes(tableId),
                ErrorCodeConstants.CODEGEN_SUB_COLUMN_NOT_EXISTS, subTable.getId());
    }
