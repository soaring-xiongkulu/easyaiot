...
| 6.4 | 匹配命中告警链 | Kafka + 库配置 | `face_library_match` / `plate_library_match` 入库 | ✅ local-only evidence — **FR-B44 ✅** `POST /video/face/matching/process` matched=true + alert_id + `face_library_match`（`library_matching_service._create_match_alert`）；profile `fr-b44-soak` `use-direct-process=false`；`logs/fr-b44-matching-alert-latest.json` **1/1**；**非 prod 绿** |
