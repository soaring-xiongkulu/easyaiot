...
| 1.4 | Face/plate matching Kafka | `video.matching.use-direct-process=false` | matching topic produce + worker 推理 + 命中告警 | ✅ local-only evidence — FR-B27：`application-fr-b27-soak.yaml` + `use-direct-process=false`；`POST /video/face/matching/publish` → `iot-face-matching` partition/offset/key=`frb27_device`；`POST /video/plate/matching/publish` → `iot-plate-matching`；`fr_b27_e2e.py` produce log OK；worker 推理 **EX**（produce-only 取证）；`logs/fr-b27-matching-kafka-latest.json`；**非 prod 绿** |
...
| 2.5 | 空间清理 cron | 磁盘超阈 | janitor / space cleanup 日志 + 对象减少 | ✅ local-only evidence — FR-B32：`frb32_device` 500B 配额 + 5×100B 对象 → `POST /video/snap/device/frb32_device/storage/cleanup` `snap_deleted_count≥1` + MinIO count 下降；`fr_b32_cleanup_e2e.py`；`logs/fr-b32-cleanup-e2e-latest.json`；**非 prod 绿** |
...
| 6.3 | YOLO pose | Python worker 或 ORT | extract/match-test 非 bypass | ✅ local-only evidence — **FR-B44 ✅** `POST /video/scenario-pose/entries/extract` 真 YOLO keypoints（max_conf≈0.99，count>0）；`match-test` similarity≥0.5 + matched；`pose_inference_cli.py` + `VIDEO/yolo26n-pose.pt`；`logs/fr-b44-pose-latest.json` **2/2**；**非 prod 绿** |
| 6.4 | 匹配命中告警链 | Kafka + 库配置 | `face_library_match` / `plate_library_match` 入库 | ✅ local-only — **FR-B44 ✅** face `POST /video/face/matching/process` **1/1**；**FR-B45 ✅** plate direct **1/1** + Kafka publish→`PlateMatchingKafkaConsumerRunner`→alert **1/1**（`logs/fr-b45-plate-matching-alert-latest.json`, `logs/fr-b45-plate-kafka-alert-latest.json`）；reExtract MinIO 代码已接线（`logs/fr-b45-re-extract-latest.json` honest_ex YOLO 暂不可用）；**非 prod 绿** |
...
| 角色 | 姓名 | 日期 | 备注 |
|------|------|------|------|
| 开发 | | 2026-08-11 | FR-B27 matching Kafka + 字段矩阵扩面 ≠ prod 绿 |
| 运维 | | | |
| 产品 | | | 永久豁免项须书面确认 |
