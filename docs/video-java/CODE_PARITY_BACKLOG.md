...
> **话术：** CODE-PARITY 波次 0 + **CP-12 完美收口尝试（2026-08-12）**：Part1 逻辑大部分已行为对齐；U4 template 通知链仍受 iot-message 网关 503 阻塞；Python 仍为对照，禁止删除。  
...
| G-11 | Deep gap cleanup | alert notify / post-matching / sink fail / FlightHub / AudioTalk / GB / directory / boot | **CP-11 PASS** (superseded evidence) | **CP-12** re-proof: U1–U3,U5–U7,U9 PASS; **U4 PARTIAL** | CP-12 | P0 **PARTIAL** |
| G-12 | Perfect gap | auto-enroll tick / FlightHub 409 flat / GB alternate wire / template notify / sink ack / matching no-plate / remote HB | **CP-12** — see `CODE_PARITY_PERFECT_GAP_PACK.md` | U4 blocked by message API 503 | CP-12 | P0 |
...
| **CP-12** perfect gap | **PARTIAL** — `logs/cp-12-u*.json`; U4 message API 503 |
