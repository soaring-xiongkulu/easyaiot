...
## 8. 现状摘要（2026-08-11 FR-B24）

- **HTTP 路由：** `route_inventory` 14 前缀 **Py≈259 / Java≈259 / diff=0**（`FR-W4` 全量核对）。
- **契约硬化：** **FR-B18 ✅** 265 路由薄探针 0 fail；**FR-B20 ✅** 16 端点深字段抽样；**FR-B21 ✅** 98 GET 信封矩阵；**FR-B22 ✅** 扩至 25 深采样；**FR-B23 ✅** 深采样 **132 pass / 0 skip**；**FR-B24 ✅** 本地 Kafka E2E + `save_time_custom` schema 修复。
- **行为：** MinIO 本地路径已取证；**Kafka 宿主机消费已取证**（`VIDEO/KAFKA_HOST_CLIENTS.md`）；ONVIF/YOLO/InsightFace 等仍为 **mini 桩**；见 `FULL_REPLACEMENT_GAP.md` §2–§4。
...
- **prod soak：** 见 [`PROD_SOAK_CHECKLIST.md`](./PROD_SOAK_CHECKLIST.md)（FR-B24 已标 **local-only** Kafka E2E 行 1.2/1.3；其余仍 ⬜）。
