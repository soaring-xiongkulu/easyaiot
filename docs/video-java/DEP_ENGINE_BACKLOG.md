...
|----|------------|---------------|-----------|----------------------|
| E-01 | **InsightFace / face 推理引擎** | `library_matching_service` / face library 调本地推理 | `LibraryMatchingProcessor` → `faceRecognitionService.isEngineAvailable()`；不可用 → `status=bypassed`（A7） | **否** |
| E-02 | **Milvus**（人脸向量库） | face 库检索 | Java/Python worker 依赖；本机 Phase1 栈未含 | **否** |
| E-03 | **`face_rec.onnx` / face 模型文件** | VIDEO/RUNTIME 侧模型 | worktree 可能有散落 onnx；**不作为已部署证据** | **否** |
| E-04 | **车牌识别模型**（若路径含 OCR，而非仅 DB `plate_entry` 查库） | plate matching 视实现可能含检测+识别 | A7 plate **DB 查库** hit/miss 已通；真 OCR 模型另计 | DB 查库 ≠ OCR；**OCR 模型旁路不可冒充引擎完成** |
