...
| 块 | 难度 | 规划体量感 | 备注 |
|----|------|--------|------|
| Phase 0 测试场 | 中 | 原估「大」 | 夹具、录制、分层 diff；对齐 rebekah certify 纪律 |
| Phase 1 Windows | 中高 | 原估「中大」 | 工程债清晰，有 rebekah 可抄 |
| Phase 2–3 契约+VIDEO 吸收 | 中 | 原估「中」 | 性价比最高的「能力不丢」 |
| Phase 4 帧内补齐 | 中高 | 原估「大」 | tracking/调度等对齐（算法任务 SAM 产品否决） |
| Phase 5 删除 | 低 | 原估「小」 | 被 certify 门禁挡住即可安全删 |

**规划粗估（历史，勿再用于排期）：** 约 **2～4 人月**（熟手；含测试场与双端回归）。该数字显著高估。

**实际耗时（以本 worktree 实施为准，2026-08）：** 从 Phase -1/0 推进到 Phase 5 certify 全绿 + Python 热路径 quarantine，**墙钟约 3 小时量级**（含 gap-close 与业务栈冒烟；非 2～3 个日历月）。

**给后续同类项目的校正：**
- 勿把「能力面宽 / 调研报告厚」直接外推成「人月级工期」；有现成 C++ 骨架、VIDEO 编排面、certify 纪律与可复用夹具时，收敛可以很快。
- 排期应以 **红清单 case 数 × 单 case 闭环** 估算，而不是复制本节历史「2～4 人月」。
- 真正拉长的通常是环境债（Win DLL/ffmpeg/设备流）与产品否决范围澄清，不是「再写一套 runtime」本身。
