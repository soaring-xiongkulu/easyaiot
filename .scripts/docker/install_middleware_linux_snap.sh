...
# 检查 Docker Compose 是否通过 snap 安装
check_docker_compose_snap() {
    if snap list docker 2>/dev/null | grep -q "^docker"; then
        # 检查 snap 的 docker-compose 插件
        if [ -f "/snap/docker/current/bin/docker-compose" ] || [ -f "/snap/docker/current/usr/bin/docker-compose" ]; then
            return 0  # 是 snap 安装
        fi
    fi
    return 1  # 不是 snap 安装
...
install_nvidia_container_toolkit() {
    ...
    if check_docker_snap; then
        # 使用 snap 重启 Docker
        if ! snap restart docker 2>/dev/null; then
            print_warning "snap Docker 重启失败，尝试使用 systemctl..."
            systemctl daemon-reload
            if ! systemctl restart snap.docker.dockerd.service 2>/dev/null; then
...
EOF
    ...
    if [ "$config_ok" = true ]; then
        ...
        # 如果是 snap 安装的 Docker，同步配置到 snap 目录
        if check_docker_snap; then
            local snap_config_dir="/var/snap/docker/current/config"
            local snap_config_file="$snap_config_dir/daemon.json"
            
            print_info "检测到 snap 安装的 Docker，同步配置到 snap 目录..."
            mkdir -p "$snap_config_dir"
            
            # 复制配置文件到 snap 目录
            if cp -f "$docker_config_file" "$snap_config_file" 2>/dev/null; then
                print_success "配置已同步到 snap 目录: $snap_config_file"
            else
                print_warning "无法同步配置到 snap 目录，可能需要手动复制"
                print_info "手动复制命令: sudo cp $docker_config_file $snap_config_file"
            fi
        fi
        ...
        if check_docker_snap; then
            # 使用 snap 重启 Docker
            print_info "正在重启 snap Docker 服务以使配置生效..."
            if snap restart docker 2>/dev/null; then
                print_success "snap Docker 服务已重启"
            else
                print_warning "snap Docker 重启失败，尝试使用 systemctl..."
                systemctl restart snap.docker.dockerd.service 2>/dev/null || true
            fi
...
download_docker_compose_from_github() {
    ...
    # GitHub 实际使用的 URL 格式
    local compose_url="https://github.com/docker/compose/releases/download/v${compose_version}/docker-compose-linux-${compose_arch}"
...
install_docker_compose() {
    ...
    # 先检查是否已安装 docker-compose
    if check_command docker-compose || docker compose version &> /dev/null; then
        # 检查版本是否符合要求
        if check_docker_compose_version; then
            print_info "Docker Compose 已安装且版本符合要求，跳过下载"
            # 如果是 snap 安装的 Docker，确保同步到 snap 目录
            if check_docker_snap; then
                local compose_path="/usr/local/bin/docker-compose"
                if [ -f "$compose_path" ]; then
                    local snap_compose_paths=(
                        "/snap/docker/current/bin/docker-compose"
                        "/snap/docker/current/usr/bin/docker-compose"
                    )
                    
                    print_info "检测到 snap 安装的 Docker，同步 docker-compose 到 snap 目录..."
                    for snap_path in "${snap_compose_paths[@]}"; do
                        local snap_dir=$(dirname "$snap_path")
                        if [ -d "$snap_dir" ]; then
                            if cp -f "$compose_path" "$snap_path" 2>/dev/null; then
                                chmod +x "$snap_path" 2>/dev/null || true
                                print_success "docker-compose 已同步到: $snap_path"
                            fi
                        fi
                    done
                fi
            ...
            print_warning "Docker Compose 已安装但版本不符合要求，将下载 v2.35.1"
        fi
    else
        print_info "未检测到 Docker Compose，将从 GitHub 下载 v2.35.1..."
    fi
    
    # 从 GitHub 下载指定版本 2.35.1
    if download_docker_compose_from_github; then
        # 如果是 snap 安装的 Docker，同步到 snap 目录
        if check_docker_snap; then
            local compose_path="/usr/local/bin/docker-compose"
            local snap_compose_paths=(
                "/snap/docker/current/bin/docker-compose"
