...
# ============================================
# EasyAIoT 中间件部署脚本 (macOS 版本)
# ============================================
# 使用方法：
#   ./install_middleware_mac.sh [命令]
#
# 可用命令：
...
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/install_middleware_mac_$(date +%Y%m%d_%H%M%S).log"

# 初始化日志文件
echo "=========================================" >> "$LOG_FILE"
echo "EasyAIoT 中间件部署脚本日志 (macOS)" >> "$LOG_FILE"
...
log_to_file() {
    ...
    # 去掉 ANSI 颜色代码（macOS 使用 BSD sed，需要 -E 选项）
...
check_and_require_git() {
    ...
    echo ""
    print_info "macOS 系统安装命令："
    print_info "  1. 使用 Homebrew 安装（推荐）："
    print_info "     brew install git"
    print_info ""
    print_info "  2. 或者从官网下载安装："
    print_info "     https://git-scm.com/download/mac"
    echo ""
...
install_nodejs20() {
    ...
    if ! brew install node@20 > /dev/null 2>&1; then
...
check_and_install_nodejs20() {
    ...
    while true; do
        ...
        case "$response" in
            [yY][eE][sS]|[yY])
                # macOS 通常不需要 root 权限安装 Node.js（通过 Homebrew）
...
# 检查 nvidia-container-toolkit 是否已安装 (macOS 版本 - 通常不需要)
check_nvidia_container_toolkit() {
    # macOS 通常不支持 NVIDIA GPU 容器，跳过检查
    print_info "macOS 系统通常不需要 nvidia-container-toolkit，跳过检查"
    return 0
}
...
install_nvidia_container_toolkit() {
    ...
    # 第四步：重启 Docker (macOS 版本 - Docker Desktop 通过应用程序管理)
    print_info "macOS 系统请手动重启 Docker Desktop 以使配置生效"
    print_info "请打开 Docker Desktop 应用程序，点击菜单栏中的 Docker 图标，选择 'Restart'"
...
# 检查并安装 nvidia-container-toolkit (macOS 版本 - 跳过)
check_and_install_nvidia_container_toolkit() {
    # macOS 通常不支持 NVIDIA GPU 容器，直接跳过
    print_info "macOS 系统通常不支持 NVIDIA GPU 容器，跳过 nvidia-container-toolkit 安装"
    return 0
}
...
configure_apt_mirror() {
    # macOS 不需要 apt 源配置
    print_info "macOS 系统不需要 apt 源配置，跳过此步骤"
    return 0
...
check_docker_permission() {
    ...
    # 检查是否有权限访问 Docker daemon（macOS 版本）
    if ! docker ps &> /dev/null; then
        ...
        echo ""
        echo "如果 Docker Desktop 未安装，请访问:"
...
# 安装 Docker (macOS 版本)
install_docker() {
    print_section "安装 Docker (macOS)"
    
    print_info "macOS 系统需要使用 Docker Desktop"
    echo ""
    print_info "安装步骤："
    print_info "  1. 访问 https://www.docker.com/products/docker-desktop"
    print_info "  2. 下载 Docker Desktop for Mac"
    print_info "  3. 安装并启动 Docker Desktop 应用程序"
    print_info "  4. 等待 Docker Desktop 完全启动后再运行此脚本"
    echo ""
    
    # 检查是否可以通过 Homebrew 安装
    if check_command brew; then
        print_info "检测到 Homebrew，可以使用 Homebrew 安装 Docker Desktop"
        echo ""
        while true; do
            echo -ne "${YELLOW}[提示]${NC} 是否使用 Homebrew 安装 Docker Desktop？(y/N): "
            read -r response
            case "$response" in
                [yY][eE][sS]|[yY])
                    ...
                    if brew install --cask docker > /dev/null 2>&1; then
                        ...
                        print_info "请打开 Docker Desktop 应用程序并等待其完全启动"
                        return 0
                    else
                        print_error "Docker Desktop 安装失败，请手动安装"
                        return 1
                    fi
                    ;;
                [nN][oO]|[nN]|"")
                    print_info "请手动安装 Docker Desktop"
                    return 1
                    ;;
                *)
                    print_warning "请输入 y 或 N"
        ...
        print_info "未检测到 Homebrew，请手动安装 Docker Desktop"
        return 1
    fi
...
EOF
    ...
    # 启动 Docker 服务 (macOS 版本)
    print_info "macOS 系统请手动启动 Docker Desktop 应用程序"
    print_info "请打开 Docker Desktop 应用程序并等待其完全启动"
...
wait_for_kafka() {
    ...
    print_info "3. 检查端口是否被占用: lsof -i :9092 或 netstat -an | grep 9092"
                ...
                if [ $has_host_process -eq 1 ]; then
                    ...
                    for port in "${conflict_ports[@]}"; do
                        ...
                        if [ "$port" = "6379" ]; then
                            ...
                            # 尝试停止常见的 Redis 服务
                            # macOS 通常没有 systemd，跳过 systemctl 检查
                            # 尝试通过进程名查找并停止
                            print_info "macOS 系统尝试通过进程名停止 Redis..."
                        ...
                        if [[ "$port" =~ ^60[34][0-9]$ ]] || [ "$port" = "6030" ] || [ "$port" = "6041" ] || [ "$port" = "6060" ]; then
                            ...
                            # macOS 通常没有 systemd，跳过 systemctl 检查
                            print_info "macOS 系统尝试通过进程名停止 TDengine..."
                            ...
                            # 5. 检查端口是否已释放
                            sleep 2
                            # macOS 使用 lsof 或 netstat 检查端口
                            if command -v lsof &> /dev/null && lsof -i :$port 2>/dev/null | grep -q LISTEN; then
                                print_warning "端口 $port 仍被占用，可能不是 TDengine 进程"
                                print_info "占用端口的进程信息:"
                                lsof -i :$port 2>/dev/null || true
                            elif command -v netstat &> /dev/null && netstat -an 2>/dev/null | grep -qE ":$port[[:space:]]|:$port$"; then
                                print_warning "端口 $port 仍被占用，可能不是 TDengine 进程"
                                print_info "占用端口的进程信息:"
                                netstat -an 2>/dev/null | grep -E ":$port[[:space:]]|:$port$" || true
                            else
...
# 显示帮助信息
show_help() {
    echo "EasyAIoT 中间件部署脚本 (macOS 版本)"
    echo ""
    echo "使用方法:"
    echo "  ./install_middleware_mac.sh [命令] [服务]"
    echo ""
    echo "可用命令:"
