...
# ============================================
# EasyAIoT 统一安装脚本 (麒麟系统版本)
# ============================================
# 使用方法：
#   ./install_linux_kylin.sh              # 无参数：打开两层交互引导（部署 / 分析）
#   ./install_linux_kylin.sh [命令]       # 直接执行命令（适合脚本/automation）
#
# 可用命令：
...
EASYAIOT_INSTALL_LABEL="${EASYAIOT_INSTALL_LABEL:-EasyAIoT 统一安装脚本 (麒麟 ARM)}"
...
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/install_linux_kylin_$(date +%Y%m%d_%H%M%S).log"

# 初始化日志文件
echo "=========================================" >> "$LOG_FILE"
echo "EasyAIoT 统一安装脚本日志 (麒麟系统)" >> "$LOG_FILE"
...
# 模块对应的安装脚本名（基础服务 / AI·VIDEO 麒麟脚本 / 其余标准脚本）
module_install_script() {
    case "$1" in
        ".scripts/docker") echo "install_middleware_linux.sh" ;;
        "AI"|"VIDEO")      echo "install_linux_kylin.sh" ;;
...
execute_module_command() {
    ...
    case "$module" in
        AI|VIDEO) print_info "执行 $module_name: $command (麒麟系统)" ;;
...
# 显示帮助信息
show_help() {
    echo "EasyAIoT 统一安装脚本 (麒麟系统版本)"
    echo ""
    echo "使用方法:"
    echo "  ./install_linux_kylin.sh                 - 打开交互式引导（推荐新手）"
    echo "  ./install_linux_kylin.sh menu            - 同上"
    echo "  ./install_linux_kylin.sh [命令] [模块]"
    echo ""
    echo "交互引导两层结构:"
    ...
    echo "  build-runtime [模块] - 构建/推送运行时镜像到远程仓库（可选 DEVICE|AI|VIDEO|WEB|VISUALIZE|TRANSFORM|PANEL）"
    ...
    echo "  - AI 和 VIDEO 模块将使用 install_linux_kylin.sh 脚本"
