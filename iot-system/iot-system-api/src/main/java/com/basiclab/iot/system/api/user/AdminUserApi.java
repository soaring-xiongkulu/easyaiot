package com.basiclab.iot.system.api.user;
...
public interface AdminUserApi extends AutoTransable<AdminUserRespDTO> {
    ...
    @PutMapping(PREFIX + "/update-experiment-status")
    @Operation(summary = "修改用户实验状态")
    CommonResult<Boolean> updateUserExperimentStatus(@RequestParam("id") Long id, @RequestParam("doExperiment") Integer doExperiment);

    @PutMapping(PREFIX + "/deduct-integral")
    @Operation(summary = "扣减用户实验积分")
    CommonResult<Boolean> updateUserExperimentIntegral(@RequestParam("id") Long id, @RequestParam("integral") Long integral);
