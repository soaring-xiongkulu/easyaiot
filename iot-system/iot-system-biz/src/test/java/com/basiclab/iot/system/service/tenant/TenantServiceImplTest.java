package com.basiclab.iot.system.service.tenant;
...
import com.basiclab.iot.system.dal.mysql.tenant.TenantMapper;
import com.basiclab.iot.system.enums.permission.RoleCodeEnum;
import com.basiclab.iot.system.enums.permission.RoleTypeEnum;
...
import static com.basiclab.iot.common.utils.collection.SetUtils.asSet;
...
public class TenantServiceImplTest extends BaseDbUnitTest {
    ...
    public void testCreateTenant() {
        ...
        when(userService.createUser(argThat(user -> {
            ...
            assertEquals("BasicLab", user.getNickname());
    ...
    public void testUpdateTenant_system() {
        // mock 数据
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> o.setPackageId(TenantDO.PACKAGE_ID_SYSTEM));
        tenantMapper.insert(dbTenant);// @Sql: 先插入出一条存在的数据
        // 准备参数
        TenantSaveReqVO reqVO = randomPojo(TenantSaveReqVO.class, o -> {
            o.setId(dbTenant.getId()); // 设置更新的 ID
    ...
    public void testGetTenantByName() {
        // mock 数据
        TenantDO dbTenant = randomPojo(TenantDO.class, o -> o.setName("BasicLab"));
        tenantMapper.insert(dbTenant);// @Sql: 先插入出一条存在的数据

        // 调用
        TenantDO result = tenantService.getTenantByName("BasicLab");
        // 校验存在
        assertPojoEquals(result, dbTenant);
    }
