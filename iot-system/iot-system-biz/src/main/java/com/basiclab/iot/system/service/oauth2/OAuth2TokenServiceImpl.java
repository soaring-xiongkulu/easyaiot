package com.basiclab.iot.system.service.oauth2;
...
import static com.basiclab.iot.common.exception.util.ServiceExceptionUtil.exception0;
...
public class OAuth2TokenServiceImpl implements OAuth2TokenService {
    ...
    @Resource
    private OAuth2AccessTokenRedisDAO oauth2AccessTokenRedisMapper;
    ...
    public OAuth2AccessTokenDO refreshAccessToken(String refreshToken, String clientId) {
        ...
        if (refreshTokenDO == null) {
            throw exception0(GlobalErrorStatus.BAD_REQUEST.getCode(), "无效的刷新令牌");
        }

        // 校验 Client 匹配
        OAuth2ClientDO clientDO = oauth2ClientService.validOAuthClientFromCache(clientId);
        if (ObjectUtil.notEqual(clientId, refreshTokenDO.getClientId())) {
            throw exception0(GlobalErrorStatus.BAD_REQUEST.getCode(), "刷新令牌的客户端编号不正确");
        }
        ...
        if (CollUtil.isNotEmpty(accessTokenDOs)) {
            ...
            oauth2AccessTokenRedisMapper.deleteList(convertSet(accessTokenDOs, OAuth2AccessTokenDO::getAccessToken));
        }

        // 已过期的情况下，删除刷新令牌
        if (DateUtils.isExpired(refreshTokenDO.getExpiresTime())) {
            oauth2RefreshTokenMapper.deleteById(refreshTokenDO.getId());
            throw exception0(GlobalErrorStatus.UNAUTHORIZED.getCode(), "刷新令牌已过期");
        }
    ...
    public OAuth2AccessTokenDO getAccessToken(String accessToken) {
        // 优先从 Redis 中获取
        OAuth2AccessTokenDO accessTokenDO = oauth2AccessTokenRedisMapper.get(accessToken);
        ...
        if (accessTokenDO != null && !DateUtils.isExpired(accessTokenDO.getExpiresTime())) {
            oauth2AccessTokenRedisMapper.set(accessTokenDO);
        }
        return accessTokenDO;
    }
    ...
    public OAuth2AccessTokenDO checkAccessToken(String accessToken) {
        ...
        if (accessTokenDO == null) {
            throw exception0(GlobalErrorStatus.UNAUTHORIZED.getCode(), "访问令牌不存在");
        }
        if (DateUtils.isExpired(accessTokenDO.getExpiresTime())) {
            throw exception0(GlobalErrorStatus.UNAUTHORIZED.getCode(), "访问令牌已过期");
        }
        return accessTokenDO;
    }
    ...
    public OAuth2AccessTokenDO removeAccessToken(String accessToken) {
        ...
        oauth2AccessTokenRedisMapper.delete(accessToken);
        // 删除刷新令牌
