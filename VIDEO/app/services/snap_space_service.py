...
def create_snap_space(space_name, save_mode=0, save_time=0, description=None, device_id=None):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"创建MinIO抓拍空间目录: {bucket_name}/{space_folder}")
        except S3Error as e:
            # 如果创建目录标记失败，记录警告但不影响整体流程（因为MinIO使用前缀即可）
            logger.warning(f"创建MinIO抓拍空间目录标记失败: {bucket_name}/{space_folder}, 错误: {str(e)}")
            ...
            try:
                ...
                if not has_objects:
                    ...
                    try:
                        ...
                        created_count += 1
                        logger.info(f"创建抓拍空间目录: {bucket_name}/{space_prefix} (空间: {space.space_name})")
                    except S3Error as e:
                        # 如果创建失败，记录错误但继续处理其他空间
                        logger.warning(f"创建抓拍空间目录失败: {bucket_name}/{space_prefix}, 错误: {str(e)}")
                        error_count += 1
                else:
                    skipped_count += 1
                    logger.debug(f"抓拍空间目录已存在，跳过: {bucket_name}/{space_prefix} (空间: {space.space_name})")
            except Exception as e:
