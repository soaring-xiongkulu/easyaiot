...
- **执行后端**: 三种任务仅支持 `executor=cpp`（本机拉起仓库 `RUNTIME` 二进制）；`executor=python` 已停用。Linux 安装入口（`install_linux.sh` / `install_linux_arm.sh` / `install_linux_kylin.sh` 的 install|update）经 `scripts/ensure_runtime_cpp.sh` 编译 RUNTIME 并挂载进 VIDEO 容器；推理默认 prefer GPU（ORT CUDA EP，失败回退 CPU）。mac 安装会跳过 RUNTIME 并提示；Windows 经 `RUNTIME/scripts/deploy.env.ps1` + MSVC 产物拉起。远程集群经 iot-node 分发 RUNTIME 后支持 executor=cpp（模型走 Ceph）。告警/心跳仍回本模块
