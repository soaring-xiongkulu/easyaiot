...
```bash
./mobile.sh status                  # 三端版本一致性 / 工具链就绪度 / 成品数量 概览
./mobile.sh build all prod          # 三端依次出包（缺哪个平台工具链会明确报错，不影响其余端）
./mobile.sh build android           # 只打 Android 生产包（默认 prod）
./mobile.sh build ios test --skip-native   # 无 Mac 环境：只做前端构建+资源同步，产出可交接的壳源码状态
./mobile.sh bump 1.0.1 101          # 发版号一次改齐 5 处（见第三节）
./mobile.sh artifacts               # 列出所有安装包成品
./mobile.sh clean all               # 清理成品（不含中间构建目录）
```
...
任何一处不一致时，`./mobile.sh status` 会标红（不一致），且各端的 make 脚本会在打包前拒绝继续——宁可不出包，也不出错包。

## 四、CI / 团队协作建议

- **前后端流水线分离**：前端资源步骤与原生工具链无关，任何 Linux runner 都能跑 `./mobile.sh build <端> <env> --skip-native`，把同步好的仓库工件交给 macOS runner（iOS）或自托管 DevEco runner（鸿蒙）做最后的原生编译。
