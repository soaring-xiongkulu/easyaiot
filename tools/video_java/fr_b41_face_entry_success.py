#!/usr/bin/env python3
"""FR-B41 — face entry multipart success (code=0 + FaceEntry.to_dict keys).

Python-first:
- face_library_service.add_entry L303-380
- face_recognition_service L288-344 extract_and_crop_largest_face
- models.py FaceEntry.to_dict L1327-1341

Artifacts: logs/fr-b41-face-entry-success-latest.{json,md}
"""
...
DISCLAIMER = (
    "FR-B41 requires real face_rec.onnx (InsightFace buffalo_l w600k_r50.onnx), "
    "Python worker health, Milvus, and a detectable face fixture. "
...
def probe_face_entry_success(base_url: str, ts: str, image: bytes, timeout: float) -> Dict[str, Any]:
    lib_id = create_library(base_url, "/video/face/libraries", f"frb41_face_{ts}", timeout)
    row: Dict[str, Any] = {
        "id": "face_entry_multipart_success",
        "path": "/video/face/libraries/{id}/entries",
        "python_source": (
            f"{_PY_FACE_SVC} add_entry L303-380 → "
            f"{_PY_FACE_REC} extract_and_crop + add_face_to_library → "
    ...
    status, body, _ = http_post_multipart(
        base_url,
        path,
        {"person_name": f"frb41_person_{ts}"},
    ...
    if ok and isinstance(data, dict):
        ...
        if image_url and str(image_url).strip():
            ...
            row["checks"].append({"check": "image_url_set", "status": "warn", "detail": "image_url empty (MinIO disabled?)"})
...
def write_artifacts(payload: Dict[str, Any]) -> Tuple[Path, Path]:
    ...
    json_path = logs / f"fr-b41-face-entry-success-{ts}.json"
    md_path = logs / f"fr-b41-face-entry-success-{ts}.md"
    latest_json = logs / "fr-b41-face-entry-success-latest.json"
    latest_md = logs / "fr-b41-face-entry-success-latest.md"
    text = json.dumps(payload, ensure_ascii=False, indent=2)
    json_path.write_text(text + "\n", encoding="utf-8")
    latest_json.write_text(text + "\n", encoding="utf-8")

    lines = [
        "# FR-B41 Face Entry Success",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="FR-B41 face entry multipart success probe")
    ...
    if server_up:
        probes.append(probe_face_entry_success(args.base_url, ts, image, args.timeout))
    else:
        probes.append(
            {
                "id": "face_entry_multipart_success",
