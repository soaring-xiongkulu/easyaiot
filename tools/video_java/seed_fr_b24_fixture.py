#!/usr/bin/env python3
"""Seed FR-B24 Kafka E2E fixture (frb24_device + spaces)."""
...
def _ensure_space(cur, table: str, code: str, bucket: str) -> int:
    ...
    cur.execute(
        ...
        RETURNING id
        """,
        (f"FR-B24 {table}", code, bucket, "fr-b24 kafka e2e seed", DEVICE_ID),
    )
...
def main() -> int:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            _ensure_device(cur)
            snap_id = _ensure_space(cur, "snap_space", "FRB24_SNAP", "snap-space")
            record_id = _ensure_space(cur, "record_space", "FRB24_RECORD", "record-space")
        conn.commit()
        out = {
            "device_id": DEVICE_ID,
        ...
        path = repo_root() / "logs" / "fr-b24-seed-fixture.json"
