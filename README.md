...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>RUNTIME C++ High-Speed Frame Executor</strong>: Moves the video-algorithm hot path from “it runs” to “it scales”—a standalone <code>RUNTIME</code> binary covers <strong>three</strong> algorithm task types: <strong>realtime</strong> (long-lived pull + Pipeline), <strong>snap</strong> (Cron SnapScheduler), and <strong>patrol</strong> (multi-device PatrolScheduler). Shared path: <strong>pull/capture → decode → YOLO inference → alert/heartbeat back to VIDEO</strong>; stages use ring queues (drop oldest when full) with observable latency and drops. VIDEO starts it via <code>executor=cpp</code>; orchestration, raw preview, and alert persistence stay in VIDEO—<strong>it does not replace the video platform layer</strong>. Formal <strong>realtime</strong> tasks <strong>default-push an independent AI overlay stream</strong> (SRS <code>ai/{device}</code>), separate from preview <code>live/</code>—operators can watch raw video and switch to boxed detections; snap/patrol emphasize structured results and alerts (may push if a URL exists, not forced long push). Prefer GPU with automatic CPU fallback. One-click C++ backend makes “orchestration in VIDEO, compute in RUNTIME” a practical mixed-language acceleration path. See <a href="RUNTIME/README.md" style="color: #3498db; text-decoration: none; font-weight: 600;">RUNTIME/README.md</a></li>
  <li><strong>RUNTIME Atomic Mode (lightweight compute nodes)</strong>: Edge boxes / cluster workers can <strong>install only the RUNTIME executor</strong>—no local VIDEO/WEB/DEVICE—lighter install, smaller blast radius, faster scale-out. Nodes aggregate alerts and heartbeats to center VIDEO via <code>VIDEO_BASE_URL</code>; the center dispatches formal realtime / snap / patrol tasks (<code>executor=cpp</code>) to the same binary, and realtime still default-pushes boxed detection streams to center/cluster SRS. Atomic ≠ never push video: atomic only means “no local business stack”; intelligence and pictures still converge at the center. Entry: <code>VIDEO_BASE_URL=http://&lt;center&gt;:6000 bash .scripts/docker/install_linux.sh runtime</code>; or WEB “workload distribute” / iot-node SSH batch install</li>
  ...
  <li><strong>Algorithm Task Management</strong>: Supports creation and management of real-time, snapshot, and patrol algorithm tasks; each task can flexibly bind frame extractors and sorters for precise video frame extraction and result sorting
    <ul style="margin: 5px 0; padding-left: 20px;">
      <li><strong>Real-Time Algorithm Tasks</strong>: Real-time video analysis with RTSP/RTMP processing; default backend <code>executor=cpp</code> (starts RUNTIME, default-pushes boxed AI streams and returns alerts/heartbeats), optional <code>python</code>—for monitoring, security, and other live scenarios</li>
      <li><strong>Snapshot Algorithm Tasks</strong>: Captured-image analysis; default can also use <code>executor=cpp</code> (RUNTIME SnapScheduler / Cron), for event backtracking and image retrieval</li>
      <li><strong>Patrol Algorithm Tasks</strong>: Multi-stream rotation and connection-pool scheduling; default can also use <code>executor=cpp</code> (RUNTIME PatrolScheduler) for batch AI patrol with “fewer connections, wider coverage”</li>
    </ul>
  </li>
  ...
  <li><strong>Separate Viewing vs Algorithm Pipelines and Tiered Bitrates</strong>: Live preview/wall viewing are decoupled from algorithm analysis. Viewing uses SRS <code>live/</code> raw streams for clarity and smoothness; the algorithm path is handled by RUNTIME (or Python) for realtime / snap / patrol, and formal realtime tasks default-push boxed detection streams to a separate <code>ai/</code> app—so analysis and viewing do not fight over one channel</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Algorithm Task Management</strong>: Supports real-time / snapshot / patrol algorithm tasks; all three can default to <code>executor=cpp</code> (generate ini and start RUNTIME); realtime also default-pushes <code>ai/</code> detection streams; <code>python</code> remains optional</li>
    <li><strong>Division with RUNTIME</strong>: VIDEO owns orchestration, raw preview (<code>live/</code>), and alert persistence; RUNTIME executes realtime / snap / patrol on the frame path, with realtime also pushing boxed detection streams (<code>ai/</code>) and hooking results back</li>
    <li><strong>Unified Result Surface</strong>: Whether Python or C++ executor, alerts POST back to this module’s <code>/video/alert/hook</code>, heartbeats update task service status, then flow into Kafka / persistence / notifications</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>C++ Frame Hot Path</strong>: Evolved from former TASK; one binary covers <strong>realtime / snap / patrol</strong>—pull or scheduled capture → decode → YOLO → structured emit; serves VIDEO <code>executor=cpp</code> without replacing VIDEO orchestration / preview / alerts</li>
    <li><strong>Three Task Shapes</strong>: Realtime long-lived Pipeline; snap Cron (SnapScheduler); patrol multi-device rotation (PatrolScheduler)—different schedules, same VIDEO alert/heartbeat contracts</li>
    <li><strong>Realtime Default AI Overlay Push</strong>: Formal realtime tasks push independent SRS <code>ai/{device}</code>, separate from preview <code>live/</code>; snap/patrol emphasize results and alerts (push optional, not forced long push)</li>
    <li><strong>Alert &amp; Heartbeat Aggregation</strong>: HTTP back to VIDEO alert hook / heartbeat (realtime &amp; snap use realtime heartbeat; patrol uses patrol heartbeat), then Kafka / persistence / notify; nodes do not keep business DB</li>
    <li><strong>Ring-Queue Pipeline</strong>: Pull/Decode, Infer, Emit on staged threads; drop oldest when full so realtime stays first</li>
    <li><strong>GPU Prefer, CPU Fallback</strong>: Default CUDA EP with automatic CPU fallback</li>
    <li><strong>Atomic Mode</strong>: Compute nodes install only RUNTIME (no VIDEO/WEB); <code>VIDEO_BASE_URL</code> aggregates to center; formal realtime/snap/patrol run on the same binary; realtime can still push detection streams to center media</li>
    <li><strong>One-Click Distribute</strong>: WEB workload distribute or iot-node SSH to <code>/opt/easyaiot/RUNTIME</code>; local VIDEO install auto-builds via <code>ensure_runtime_cpp.sh</code></li>
  </ul>
</td>
</tr>
<tr>
...
<p style="font-size: 14px; line-height: 1.8; color: #555; margin: 10px 0;">
The platform supports real-time, snapshot, and patrol algorithm tasks: all three can default to the <strong>RUNTIME C++ high-speed executor</strong> (<code>executor=cpp</code>)—realtime long-lived pull/decode/YOLO with <strong>default boxed AI stream push</strong>; snap Cron capture recognition; patrol multi-stream rotation. Alerts and heartbeats return to VIDEO’s existing loop; Python remains an optional backend. Compute nodes support <strong>RUNTIME atomic mode</strong> (executor-only install, alerts aggregate to the center). Through algorithm task management, flexible frame extraction and sorting strategies are achieved, with each task able to bind independent frame extractors and sorters. Combined with model service cluster inference capabilities, millisecond-level response and high availability are ensured. Additionally, two defense strategies are provided: full defense mode and half defense mode, allowing flexible configuration of monitoring rules for different time periods, achieving precise time-based intelligent monitoring and alerting.
</p>
<p style="font-size: 14px; line-height: 1.8; color: #555; margin: 10px 0;">
