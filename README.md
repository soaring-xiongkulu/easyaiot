...
### ⌨️ TERMINAL: 30+ Protocols Behind One Entry—No More Toolbelt for On-Call Troubleshooting

<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
On-site troubleshooting rarely stalls on the fault itself—it stalls on stitching tools together: one client for SSH, a separate install of Navicat for databases, another app for Redis, a standalone SFTP client for transfers, and raw server logins just to run docker commands—accounts, tunnels, and jump hosts configured per machine, and reconfigured on every new one. TERMINAL, integrated and renamed from the open-source uniTerm, covers <strong>30+ protocols</strong> across five ops scenarios—remote terminal, remote desktop, file transfer, databases, and containers—and connects the full EasyAIoT middleware stack with one click from the left panel; packaged with Wails v3 (Go + Vue 3) as a single app for Windows / macOS / Linux, or run in server mode over pure HTTP—turning “a toolbelt of tools and a pile of clients” into “one entry, every site.”
</p>

<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>Remote terminals & desktops</strong>: SSH / Telnet / Mosh remote terminals plus local and serial terminals, with password / key auth and <strong>SSH tunnel port forwarding</strong> straight into intranets; RDP / VNC / SPICE / X11 remote desktops—operate Windows machines with no extra remote-control installs</li>
  <li><strong>File transfer</strong>: dual-pane browsing over SFTP / SCP / FTP / SMB / WebDAV / S3 with <code>rz</code>/<code>sz</code> and drag-and-drop upload—pull log bundles and evidence files back on the spot</li>
  <li><strong>Database clients</strong>: MySQL / PostgreSQL / Oracle / SQL Server / Redis / MongoDB out of the box—table structures, SQL queries, result-set editing, and visual Redis key management in one place, with no more side-installed Navicat / RDM</li>
  <li><strong>Containers & monitoring</strong>: Kubernetes / Docker / Podman / nerdctl container terminals, logs, and start/stop on one screen, plus real-time CPU / memory / disk / network / process monitoring to locate resource bottlenecks</li>
</ul>

| | | |
|:---:|:---:|:---:|
| ![SSH terminal](.image/banner/terminal/terminal_1002.png) | ![Containers](.image/banner/terminal/terminal_1000.png) | ![Container logs](.image/banner/terminal/terminal_1001.png) |
| ![Service logs](.image/banner/terminal/terminal_1008.png) | ![Table structures](.image/banner/terminal/terminal_1003.png) | ![Multi-database](.image/banner/terminal/terminal_1005.png) |
| ![SQL query](.image/banner/terminal/terminal_1004.png) | ![S3 files](.image/banner/terminal/terminal_1007.png) | ![Redis keys](.image/banner/terminal/terminal_1006.png) |
...
<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
EasyAIoT comprises core modules including WEB, APP, DEVICE, EDGE, SENTINEL, VIDEO, RTC, AI, RUNTIME, POST, VISUALIZE, TWIN, TRANSFORM, PANEL, IDEA, HARNESS, SITE, and TERMINAL, plus COMPILE multi-platform packaging and delivery:
</p>
...
<tr>
<td style="padding: 15px; border: 1px solid #e0e0e0; vertical-align: top;"><strong>TWIN Module</strong></td>
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>Multi-industry digital twins</strong>: Four built-in Three.js scenes for an industrial park, smart campus, smart port, and net-zero energy base, with project-specific themes and operational views</li>
    <li><strong>3D navigation and spatial positioning</strong>: Rotate, zoom, and smoothly switch preset viewpoints; device locations, online/warning/offline states, and live alerts highlight together in the scene</li>
    <li><strong>Building and floor operations</strong>: Focus a building, switch floors, isolate devices on the active floor, and track device, online, warning, and unbound counts together</li>
    <li><strong>Device and data binding</strong>: In edit mode, bind 3D objects and points to sensors, cameras, PLCs / edge gateways via MQTT, Modbus TCP, OPC UA, GB28181, or ONVIF</li>
    <li><strong>Replaceable model and data layers</strong>: The MVP currently uses procedural scenes and simulated live data; stable device-ID mapping allows later replacement with Blender / GLB assets and EasyAIoT API, MQTT over WebSocket, or backend WebSocket sources</li>
  </ul>
</td>
</tr>
<tr>
...
<tr>
<td style="padding: 15px; border: 1px solid #e0e0e0; vertical-align: top;"><strong>TERMINAL Module</strong></td>
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
...
<td style="padding: 15px; border: 1px solid #e0e0e0; color: #444; line-height: 1.8;">To advance EasyAIoT in map visualization and spatial intelligence, independently contributed the complete implementation of Tianditu spatial visualization capabilities, covering national Tianditu basemap integration, camera and alarm device placement, map distribution views, location search and batch coordinate import, automatic alarm event mapping, person/vehicle trajectory tracking, and mobile device track playback—bringing the platform's "Tianditu spatial visualization and map-based analysis" capability from design to production-ready, usable form. Building on that, he further contributed forward-looking reference code for "drawing tracking trajectories along actual roads"—addressing the common pain point that raw capture-point connections cut straight across buildings and water and deviate from real movement paths. He proposed a technical approach that treats consecutive capture points as origin–destination pairs, queries the map route-planning service segment by segment to match passable roads (driving strategy for vehicles, walking strategy for persons) and overlays the road-snapped paths, together with key implementation references for parsing route-planning results and gracefully degrading unmatched segments. This upgrades person/vehicle trajectories from "two-point straight lines" to "street-level reconstruction," making spatial-temporal analysis conclusions far closer to real movement routes and markedly improving their readability, credibility, and practical value. His selfless sharing of a reusable reference implementation also cleared technical barriers for the community and greatly lowered the reproduction threshold for similar capabilities.</td>
</tr>
<tr>
