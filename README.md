...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>RUNTIME C++ High-Speed Execution Layer</strong>: Pulls “watch / compute / push” out of interpreted-language paths into a <strong>native C++ binary</strong> that owns <strong>pull → decode → infer → alert callback → boxed push</strong>. Versus Python executors, it avoids GIL and multi-process orchestration overhead—on the same hardware you get <strong>more channels per box, lower end-to-end latency, and less CPU/memory</strong>. Realtime / snap / patrol and multi-channel raw forward share one execution stack; default <code>executor=cpp</code>. In practice: keep dozens of NVR channels clear first, then enable AI only on critical channels; switch raw preview and boxed judgment without either path dragging the other down; VIDEO/WEB keep orchestration, archiving, and access control while C++ focuses on throughput and latency—scale channels without replacing the whole appliance</li>
  <li><strong>High-Performance Stream Forwarding</strong>: Built for the real delivery need to “fill the video wall / split view first, without turning AI on for every channel”—batch NVR channels and building endpoints can create forwarding tasks in one click. Default path is <strong>RUNTIME C++</strong> high-performance (compatibility mode still available) so multi-channel raw video stays on wall at lower resource cost. Raw viewing and AI judgment can run on the same device in parallel; when algorithm tasks start/stop, forwarding policy follows automatically. Side-by-side preview shortens acceptance by comparing latency and overlay quality directly</li>
  <li><strong>Atomic Mode: Compute Box Installs Only the C++ Executor</strong>: Edge boxes / cluster workers can <strong>install only the RUNTIME binary</strong>—no local VIDEO / WEB / DEVICE—smaller install, narrower blast radius, scale-out like “add compute” not “install another platform”. Nodes aggregate alerts and heartbeats to center VIDEO; the center dispatches realtime / snap / patrol (<code>executor=cpp</code>) to the same C++ process, and realtime still default-pushes boxed detection streams back to center SRS. Atomic = no local business stack, not “never push video”; intelligence and pictures still converge at the center. One-liner: <code>VIDEO_BASE_URL=http://&lt;center&gt;:6000 bash .scripts/docker/install_linux.sh runtime</code>; or WEB “workload distribute” batch install</li>
  ...
  <li><strong>Algorithm Task Management</strong>: Supports creation and management of real-time, snapshot, and patrol algorithm tasks; each task can flexibly bind frame extractors and sorters for precise video frame extraction and result sorting
    <ul style="margin: 5px 0; padding-left: 20px;">
      <li><strong>Real-Time Algorithm Tasks</strong>: Real-time video analysis with RTSP/RTMP processing; default backend <code>executor=cpp</code> (starts <strong>RUNTIME C++</strong>, default-pushes boxed AI streams and returns alerts/heartbeats), optional <code>python</code> compatibility path</li>
      <li><strong>Snapshot Algorithm Tasks</strong>: Captured-image analysis; default can also use <code>executor=cpp</code> (C++ SnapScheduler / Cron)</li>
      <li><strong>Patrol Algorithm Tasks</strong>: Multi-stream rotation and connection-pool scheduling; default can also use <code>executor=cpp</code> (C++ PatrolScheduler) for “fewer connections, wider coverage”; nodes can run atomic mode with executor-only install</li>
    </ul>
  </li>
  ...
  <li><strong>Stream Forwarding</strong>: View live camera feeds without enabling AI; batch push and auto NVR channel tasks work out of the box. Default <strong>RUNTIME C++</strong> high-performance path keeps multi-channel raw video cheaper on wall—“fill the wall first, AI on critical channels”; special sites can switch to Python/FFmpeg compatibility mode</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Node Roles</strong>: Supports compute, media, and hybrid roles for cross-node scheduling and elastic scaling; compute nodes can use <strong>RUNTIME atomic mode</strong> (C++ executor only) to take center-dispatched realtime / snap / patrol and high-performance forward</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Orchestration vs Execution</strong>: VIDEO owns device orchestration, raw preview, alert archiving, and start/stop; heavy lifting goes to <strong>RUNTIME C++</strong> (inference and high-performance forward)—“governed” and “fast” stay separate so channel count and latency are not dragged by Python orchestration</li>
...
<tr>
<td style="padding: 15px; border: 1px solid #e0e0e0; vertical-align: top;"><strong>RUNTIME Module (C++)</strong></td>
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>Native C++ Execution</strong>: One binary owns pull, decode, ONNX inference, alert/heartbeat callbacks, and boxed push; default <code>executor=cpp</code>—more channels per box, lower latency, leaner resources than Python paths</li>
    ...
    <li><strong>Resource-Efficient Multi-Channel Raw Wall</strong>: Stream forward defaults to the C++ high-performance path so “many channels watch raw, few channels run AI” can coexist on NVR sites</li>
    ...
    <li><strong>Atomic Mode</strong>: Compute boxes / workers install only the C++ executor—no VIDEO/WEB/DEVICE; center orchestrates, nodes execute—scale like adding compute, not another full platform</li>
...
<p style="font-size: 14px; line-height: 1.8; color: #555; margin: 10px 0;">
The platform supports real-time, snapshot, and patrol algorithm tasks: all three can default to the <strong>RUNTIME C++ high-speed execution layer</strong> (<code>executor=cpp</code>)—a native binary for long-lived pull, decode, YOLO inference, and result callbacks; realtime <strong>default-pushes boxed AI detection streams</strong>; snap uses Cron capture; patrol rotates multi-channel coverage; stream forward can use the same C++ path for resource-efficient raw walls. Versus the Python compatibility backend, C++ holds up better under high channel counts and low latency. Alerts and heartbeats return to VIDEO; compute nodes can enable <strong>atomic mode</strong>—C++ executor only, business surface stays at the center—so edge boxes join light and scale by adding nodes. Algorithm task management keeps frame extraction and sorting flexible; model-service cluster inference ensures millisecond response and high availability. Full and half defense modes support precise time-based monitoring and alerting.
</p>
<p style="font-size: 14px; line-height: 1.8; color: #555; margin: 10px 0;">
