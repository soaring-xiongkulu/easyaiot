...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>RUNTIME C++ High-Speed Frame Executor</strong>: Evolves the former TASK into a standalone <code>RUNTIME</code> module dedicated to the real-time video hot path—pull, hardware/software decode, YOLO inference, and structured result emission. Stages are decoupled by ring queues (drop oldest when full) with observable end-to-end latency and drop metrics; VIDEO starts it per task via <code>executor=cpp</code>, while alerts and heartbeats still use VIDEO’s existing hook / Kafka loop, and preview relay plus task orchestration stay in VIDEO—<strong>it does not replace or bypass the video platform layer</strong>. Algorithm tasks use <code>executor=cpp</code> only (<code>executor=python</code> retired after runtime-parity certify), making “orchestration in VIDEO, compute in RUNTIME” the sole production acceleration path</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Algorithm Task Management</strong>: Supports real-time / snapshot / patrol algorithm tasks; execution backend is <code>executor=cpp</code> only (VIDEO generates ini and starts RUNTIME; <code>executor=python</code> retired)</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Linux Edge Friendly</strong>: CMake produces a standalone binary deployable with conda / system deps; Windows MSVC + vendor path also supported</li>
  </ul>
</td>
</tr>
<tr>
