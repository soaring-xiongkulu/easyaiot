...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>Atomic Mode: Compute Box Installs Only the Executor</strong>: Edge boxes / cluster workers can <strong>install only the RUNTIME binary</strong>—no local VIDEO / WEB / DEVICE—smaller install, narrower blast radius, scale-out like “add compute” not “install another platform”. Nodes aggregate alerts and heartbeats to center VIDEO; the center dispatches realtime / snap / patrol (<code>executor=cpp</code>) to the same execution process, and realtime still default-pushes boxed detection streams back to center SRS. Atomic = no local business stack, not “never push video”; intelligence and pictures still converge at the center. One-liner: <code>VIDEO_BASE_URL=http://&lt;center&gt;:6000 bash .scripts/docker/install_linux.sh runtime</code>; or WEB “workload distribute” batch install</li>
  ...
  <li><strong>RUNTIME Atomic Edge Compute</strong>: For wide-area deployment and compute-box scale-out—nodes install only the RUNTIME executor; alerts flow via MQTT to iot-sink, heartbeats to VIDEO; no need to stack a full business stack at every site. The former EDGE federated edge suite has been removed; this is the unified path</li>
  ...
  <li><strong>Algorithm Task Management</strong>: Supports creation and management of real-time, snapshot, and patrol algorithm tasks; each task can flexibly bind frame extractors and sorters for precise video frame extraction and result sorting
    <ul style="margin: 5px 0; padding-left: 20px;">
      ...
      <li><strong>Patrol Algorithm Tasks</strong>: Multi-stream rotation and connection-pool scheduling; default can also use <code>executor=cpp</code> (RUNTIME PatrolScheduler) for “fewer connections, wider coverage”; nodes can run atomic mode with executor-only install</li>
    </ul>
  </li>
...
<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
EasyAIoT comprises core modules including WEB, APP, DEVICE, NODE, VIDEO, RTC, AI, RUNTIME, VISUALIZE, TRANSFORM, PANEL, and SITE, plus COMPILE multi-platform packaging and delivery (EDGE removed; edge uses RUNTIME atomic mode):
</p>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Node Roles</strong>: Supports compute, media, and hybrid roles for cross-node scheduling and elastic scaling; compute nodes can use <strong>RUNTIME atomic mode</strong> (executor only) to take center-dispatched realtime / snap / patrol and high-performance forward</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Atomic Mode</strong>: Compute boxes / workers install only the executor—no VIDEO/WEB/DEVICE; center orchestrates, nodes execute—scale like adding compute, not another full platform</li>
...
<tr>
<td style="padding: 15px; border: 1px solid #e0e0e0; vertical-align: top;"><strong>RUNTIME Atomic Edge</strong></td>
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>EDGE module removed</strong>: Edge compute unified via <strong>RUNTIME atomic mode</strong> (executor only)</li>
    ...
    <li><strong>Management plane HTTP heartbeat</strong>: Task liveness still reports to VIDEO; start/stop and task table managed by VIDEO</li>
