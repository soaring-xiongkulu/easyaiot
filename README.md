...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>Linked Detection Region + Defense Time Period + Event Alert, Triple-Link Alert</strong>: Supports intelligent linkage mechanism between detection regions, defense time periods, and event alerts. When a detected event simultaneously meets the specified detection region range, falls within the defense time period, and matches the alert event type, the system will automatically trigger a linked alert, achieving precise spatiotemporal condition alerts, effectively reducing false positives, and improving alert accuracy and practicality</li>
