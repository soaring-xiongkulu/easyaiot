...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>RUNTIME High-Performance Video Executor</strong>: Moves on-site video from “can watch / can analyze” to “more channels stay up, duty keeps up”—one execution capability covers <strong>realtime judgment, scheduled snapshots, and multi-channel patrol</strong>, while also powering <strong>multi-channel raw video on wall</strong>. In practice: keep dozens of NVR channels clear and smooth first, then enable AI only on critical channels; switch between raw preview and boxed judgment without either path dragging the other down; keep overlays readable so false alarms do not distract operators. The platform still owns device orchestration, alert archiving, and access control; the executor focuses on throughput and latency—so scaling channels and scenes does not require replacing the whole appliance. See <a href="RUNTIME/README.md" style="color: #3498db; text-decoration: none; font-weight: 600;">RUNTIME/README.md</a></li>
  <li><strong>High-Performance Stream Forwarding</strong>: Built for the real delivery need to “fill the video wall / split view first, without turning AI on for every channel”—batch NVR channels and building endpoints can create forwarding tasks in one click. High-performance mode is the default so multi-channel raw video stays on wall at lower resource cost; special sites can still switch to compatibility mode. Raw viewing and AI judgment can run on the same device in parallel: operators can watch unlabeled live video or switch to boxed results; when algorithm tasks start/stop, forwarding policy follows automatically—fewer manual config edits and fewer round-trips of joint debugging. Side-by-side preview shortens acceptance by comparing latency and overlay quality directly</li>
  ...
  <li><strong>Stream Forwarding</strong>: Watch live camera video without enabling AI; batch push and auto-create NVR channel tasks work out of the box. High-performance mode keeps multi-channel raw video cheaper to wall-mount—“fill the wall first, enable AI on key channels later”; compatibility mode remains for special sites</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Orchestration vs Execution</strong>: VIDEO owns device orchestration, raw preview, alert archiving, and start/stop; RUNTIME focuses on high-throughput analysis and efficient multi-channel raw display—“governed” and “fast” stay separate</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>High-Performance Video Execution</strong>: Accelerates realtime judgment, scheduled snapshots, and multi-channel patrol, and keeps multi-channel raw video efficient on wall; the platform still owns orchestration and alert archiving while the executor focuses on throughput and latency</li>
    ...
    <li><strong>Raw + Judgment Dual Views</strong>: Realtime tasks can show boxed results separately from raw preview; overlays stay readable to reduce false-alarm distraction</li>
    <li><strong>Resource-Efficient Multi-Channel Raw Wall</strong>: High-performance forwarding is built for “fill the wall first,” so “many channels watch raw, few channels run AI” can coexist on NVR sites</li>
    <li><strong>Alerts &amp; Status Back to Center</strong>: Analysis and forwarding status aggregate to the platform for persistence and notification; nodes do not stack business data locally</li>
    ...
    <li><strong>Atomic Mode</strong>: Edge nodes can install execution only, without the full business stack; results still return to the center for unified judgment</li>
    <li><strong>One-Click Distribute</strong>: Console or node channels can batch-distribute executors; local install can prepare the runtime automatically</li>
  </ul>
</td>
</tr>
<tr>
