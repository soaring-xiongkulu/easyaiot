...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>RUNTIME C++ High-Speed Frame Executor</strong>: Evolves the former TASK into a standalone <code>RUNTIME</code> module dedicated to the real-time video hot path—pull, hardware/software decode, YOLO inference, and structured result emission. Stages are decoupled by ring queues (drop oldest when full) with observable end-to-end latency and drop metrics; VIDEO starts it per task via <code>executor=cpp</code>, while alerts and heartbeats still use VIDEO’s existing hook / Kafka loop, and preview relay plus task orchestration stay in VIDEO—<strong>it does not replace or bypass the video platform layer</strong>. Default <code>executor=python</code> keeps production compatibility; switch to the C++ backend in one click when peak throughput is required, making “orchestration in VIDEO, compute in RUNTIME” a practical mixed-language acceleration path</li>
  ...
  <li><strong>Algorithm Task Management</strong>: Supports creation and management of real-time, snapshot, and patrol algorithm tasks; each task can flexibly bind frame extractors and sorters for precise video frame extraction and result sorting
    <ul style="margin: 5px 0; padding-left: 20px;">
      <li><strong>Real-Time Algorithm Tasks</strong>: Used for real-time video analysis with RTSP/RTMP stream processing; execution backend can be <code>python</code> (default, existing run_deploy) or <code>cpp</code> (starts the RUNTIME binary), suitable for monitoring, security, and other real-time scenarios</li>
      ...
      <li><strong>Patrol Algorithm Tasks</strong>: Multi-stream round-robin with connection-pool scheduling—“fewer connections, wider coverage” for batch AI patrol</li>
    </ul>
  </li>
...
<p style="font-size: 14px; line-height: 1.8; color: #555; margin: 15px 0;">
EasyAIoT is not actually one project; it comprises multiple independently deployable sub-projects (WEB, DEVICE, VIDEO, AI, and more).
</p>
...
<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
EasyAIoT comprises core modules including WEB, DEVICE, NODE, VIDEO, AI, RUNTIME, EDGE, VISUALIZE, TRANSFORM, PANEL, plus COMPILE multi-platform packaging and delivery:
</p>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Algorithm Task Management</strong>: Supports real-time / snapshot / patrol algorithm tasks; real-time tasks can set execution backend <code>executor=python|cpp</code> (default python; when cpp, this module generates ini and starts RUNTIME)</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>C++ Frame Hot Path</strong>: Evolved from former TASK; focuses on pull → decode → YOLO inference → result emit for VIDEO <code>executor=cpp</code> real-time tasks (does not replace VIDEO orchestration / preview / alert surfaces)</li>
    <li><strong>Ring-Queue Pipeline</strong>: Pull/Decode, Infer, and Emit on staged threads; frame ring drops oldest when full so realtime stays first and decode is not blocked by inference</li>
    <li><strong>VIDEO Contract Alignment</strong>: Alerts POST <code>/video/alert/hook</code>, heartbeats POST <code>/video/algorithm/heartbeat/realtime</code>, <code>/health</code> exposes drop and latency metrics</li>
    <li><strong>Linux Edge Friendly</strong>: CMake produces a standalone binary deployable with conda / system deps; default python executor remains an instant fallback</li>
  </ul>
</td>
</tr>
<tr>
