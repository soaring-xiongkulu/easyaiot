...
#### 项目定位：支持百级以下摄像头接入，推理，采集，分析，告警，存储，标注，依托于Kubernetes云原生编程，实现GPU池化/切割、模型量化、算子定制、AI服务集群化部署（支持RTSP流、视频、图像、文本、语音等）

#### 注：如果是接入百~万级摄像头，请关注姊妹项目MegaAIoT，量级不同，实现难度，思维方式也不同
