...
| | | | |
|:---:|:---:|:---:|:---:|
| <img src=".image/certificates/gitee-oss-award-2025-top1.jpg" width="200" alt="Gitee 2025 年度開源專案 · 工業軟體賽道 Top1"> | <img src=".image/certificates/gitee-2000-stars.jpg" width="200" alt="Gitee 2000+ Stars"> | <img src=".image/certificates/gitee-gvp-2025.jpg" width="200" alt="Gitee GVP 2025 · 最有價值開源專案"> | <img src=".image/certificates/csdn-blog-expert.jpg" width="200" alt="CSDN 部落格專家"> |

## 🚀 快速開始

<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
打開 README 心裏嘀咕：「我這破機器……裝得動嗎？」——<strong>能。先別慫。</strong>
</p>

<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
最輕一檔，容器總佔用大約就 <strong>1 GB</strong>。攝像頭接入、即時分析、智能告警，小機器也能閉環——舊筆記本先爽一把，上癮了再升檔也不遲。
</p>
...
### 🏷️ 大模型智能標註：用業務語言定義場景，把現場知識訓練成專屬小模型

傳統自動標註通常只能輸入「人、車、安全帽」等類別詞，卻難以準確表達客戶真正關心的現場規則。EasyAIoT 新增基於 <strong>HARNESS 大模型底座</strong>的獨立大模型智能標註流程：使用者以完整自然語言描述場景，大模型理解物件、動作、關係、條件與排除項並產生結構化標註；人工審核後，平臺自動完成 YOLO 小模型訓練、發佈、綁定與批量回標。

- <strong>更貼近真實需求</strong>：不只表達「是什麼」，還能描述位置、動作、關係、成立條件及例外。
- <strong>定製化冷啟動</strong>：無需成熟模型或大量人工樣本，描述場景並審核首批結果即可開始訓練。
- <strong>理解力與效率兼得</strong>：大模型理解少量樣本的複雜語義，小模型承擔低成本、高速、邊緣化推理。
- <strong>完整可追溯閉環</strong>：場景描述 → 候選標註 → 人工審核 → 小模型訓練 → 自動發佈綁定 → 批量回標。

<strong>與 SAM3 的區別：</strong>SAM3 使用簡短視覺提示詞，擅長定位或分割「像這個目標的物體」；大模型接收完整業務描述，判斷「什麼條件下的哪個目標才是真正需要標註的」。通用目標快速提取優先使用 SAM3；涉及動作、關係、條件、例外或行業語義時優先使用大模型智能標註。
...
### 🛰️ SENTINEL：集羣節點哨兵，讓「納管即就緒、調度按真能力」

<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
聯邦擴容最怕「機器加進來了，卻不知道能不能接活」——視頻分析、推流轉發、模型訓練各要不同就緒條件，任務派下去才發現環境缺件，現場反覆登機裝依賴，驗收卡在「裝完跑不起來」。EasyAIoT 新增獨立 <strong>SENTINEL 模組</strong>，作為跟著每臺調度節點走的<strong>業務就緒哨兵</strong>：按勾選業務持續摸清本機是否真正可接活，把可調度能力匯聚到管控臺；缺件時可自動請求環境補齊，讓運維少登機、任務少踩坑、擴容交付當天就能驗收。
</p>

<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>按業務認能力</strong>：視頻分析、推流轉發、直播接入、模型訓練、大模型、智能標註等——勾什麼業務，就盯什麼就緒度，不再按角色猜「這臺機器大概能干啥」</li>
  <li><strong>納管即監測</strong>：節點納入平臺後自動拉起哨兵巡檢，擴容當天就能看見「能不能接活」，縮短「加機器—等聯調—才發現不能用」的空窗</li>
  <li><strong>真能力驅動調度</strong>：只有環境真正就緒的節點進入可調度池，減少「派了跑不起來」的返工與客訴</li>
  <li><strong>缺件自助補齊</strong>：期望環境缺失時可自動請求平臺補齊，把現場手工裝機壓縮成平臺側閉環，值守少登伺服器</li>
  <li><strong>斷網也能擴</strong>：無公網或受限網絡現場可離線納管與環境同步，邊端擴容不被外網卡住</li>
...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>告警工單（AI 告警責任閉環）</strong>：讓「AI 報出告警」真正落到「有人負責、有流程跟進、有結果可查」。告警落庫後按<strong>路由規則</strong>自動匹配（告警對象/事件/任務/設備/邊緣節點多條件組合），命中即自動生成<strong>告警工單</strong>併發起處理流程——無需人工盯屏派單，責任直接到人。工單依託平台內置的<strong>流程編排能力</strong>（可視化流程設計器：審批、會簽、抄送、條件分支、並行分支、超時提醒等節點自由組合），支持通過/拒絕/退回/委派/轉辦/加簽等審批動作與站內信/APP 待辦提醒（deepLink 一鍵直達審批詳情），每條告警的處置狀態、當前責任人、耗時全程留痕，形成「告警—工單—處置—覆核」完整閉環；存量告警也可手動補發起工單兜底，避免漏處置</li>
...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>六語言工具鏈對齊本機</strong>：預裝 <strong>JDK 21</strong>、Node 22、Python、Go、CMake/C++、.NET 8，覆蓋 WEB / DEVICE / AI / RUNTIME / EDGE 的閱讀、輕改與模組級驗證</li>
...
### 🔌 LLM 統一閘道 — 一套協定，接入全部主流大模型廠商

<p style="font-size: 14px; line-height: 1.8; color: #555; margin: 15px 0;">
平台將各大廠商大模型統一收斂到一套 OpenAI 相容協定（<code>/v1/chat/completions</code>）。同一時刻僅啟用 1 個大模型，頁面內全部 AI 能力（圖像/影片理解、自動標註、對話）一鍵切換；新增廠商零程式碼——模板註冊表加一條即可。首次部署自動播種預置模板資料（佔位金鑰 <code>sk-placeholder-*</code>、未啟用），編輯中填入真實 API 金鑰即可啟用，全流程有介面引導。
</p>

| 廠商 | 模板識別 | 模型識別（建議清單） | 說明 |
| :-- | :-- | :-- | :-- |
| DeepSeek | `deepseek` | deepseek-v4-flash、deepseek-v4-pro、deepseek-v4-flash-vision-exp | 100 萬上下文，官方 API |
| 阿里雲百煉 | `dashscope` | qwen3.8-max、qwen3.7-max/plus/flash、qwen3.6-plus/flash、qwen3.5-plus/flash、qwen3.5-omni-plus/flash、qwen3-vl-plus/flash、qwen-vl-max | OpenAI 相容模式端點 |
| 智譜 GLM | `zhipu` | glm-5.3、glm-5.2、glm-5.1、glm-5-turbo、glm-5、glm-4.7、glm-4.7-flash、glm-4.5-flash、glm-4-plus/flash/air、glm-4v-plus/flash、glm-ocr | v4 端點 |
| OpenAI | `openai` | gpt-5.6-sol、gpt-5.6、gpt-5.6-sol-pro、gpt-5.6-terra、gpt-5.6-luna、gpt-5.5、gpt-5.4-mini、gpt-4.1-mini | 自動相容 max_completion_tokens |
| Kimi（月之暗面） | `kimi` | kimi-k2.6、kimi-k2.7-code | K2 系列強制 temperature=1 |
| Claude（Anthropic） | `claude` | claude-opus-4-7/4-6/4-5、claude-sonnet-4-6/4-5、claude-haiku-4-5 | 官方為 Messages 協定，經 OpenAI 相容轉發/閘道接入 |
| Anthropic 相容 | `anthropic` | — | 自填 OpenAI 相容轉發端點 |
| 自訂 | `custom` | — | 任意 OpenAI 相容端點 |

<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>預置模板資料</strong>：全新部署自動播種 6 條模板（DeepSeek / 百煉 / GLM / OpenAI / Kimi / Claude）；佔位金鑰未替換前，啟用與連線測試均被攔截引導，不會出現無效 401</li>
  <li><strong>其他服務直接使用</strong>：平台將當前啟用模型暴露為 OpenAI 相容端點（<code>POST /v1/chat/completions</code>、<code>GET /v1/models</code>），任何 OpenAI SDK 憑 Bearer Token 即可呼叫</li>
...
<p style="font-size: 14px; line-height: 1.8; color: #555; margin: 15px 0;">
在 LLM 統一閘道之上，平台將業務文件轉化為<strong>可問答的知識資產</strong>，並封裝為可對話的<strong>智能體</strong>——從知識沉澱、檢索回答到頁面內與 IDE 內的智能助手，一條鏈路打通。
</p>

<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>知識文件 → 片段 → 知識集</strong>：上傳 TXT / Markdown / CSV / JSON / LOG 檔案（UTF-8、≤10MB），自動切片寫入 Milvus 向量索引；片段可編輯、可獨立啟停檢索，跨文件組合成為可複用的知識集</li>
  <li><strong>RAG 專家（智能體）</strong>：組合多個知識集、設定角色指令與歡迎語即可開聊——回答引用<code>【資料 N】</code>，資料不足不編造，模型不可達時自動降級為純檢索</li>
  <li><strong>混合檢索</strong>：Milvus 向量檢索與關鍵字（類 BM25）打分融合；內建 <code>local-hash</code> 離線 Embedding，無外部 Key 也能跑通全鏈路</li>
  <li><strong>AI 助手</strong>：業務頁懸浮助手即問即查；HARNESS 對話式助手嵌入 IDEA 在線 IDE，拖檔自動 <code>@</code>、分屏共創，並經 MCP + Cursor Skill 複用到 Cursor 等開發環境</li>
</ul>

### 🧠 大模型告警後處理（LLM 研判）— 讓「檢出」變成「可信」

<p style="font-size: 14px; line-height: 1.8; color: #555; margin: 15px 0;">
傳統視覺演算法解決了「看見」，卻把「信不信」留給了人——安全帽、越線、明火、睡崗的偵測框每天刷屏，值守要逐條點開圖片人工複核，誤報淹沒真警情，真警情又怕漏掉。EasyAIoT 把<strong>大模型語意理解接入告警鏈路</strong>：演算法任務可配置<strong>大模型後處理規則</strong>，命中規則的告警自動進入獨立研判佇列，由多模態大模型逐條查看現場圖片（或短影片），給出<strong>「確認成立 / 判定誤報」的結論、信心度與理由</strong>並回寫告警——值守從「逐圖盯幀」變成「只看結論」，把高頻誤報擋在通知之外。
</p>

<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>獨立非同步研判佇列</strong>：規則匹配後經 Kafka 投遞、非同步執行，研判慢、模型抖動都不阻塞告警落庫與通知鏈路；研判失敗只記日誌，絕不拖垮主流程</li>
  <li><strong>抽檢與限流控成本</strong>：命中規則的告警按<strong>抽檢比例</strong>（如 10% = 每 10 條抽 1 條）進入大模型佇列，疊加<strong>同任務最小研判間隔</strong>防止高頻事件重複呼叫；未命中抽檢或被限流的告警自動標記「未抽檢 / 限流跳過」，大模型費用與告警量脫鉤，量再大成本也可控</li>
  <li><strong>圖片 / 影片雙研判</strong>：預設看圖研判，可切換短影片研判並設定事件前後視窗與切片最大時長；錄影缺失時自動回退圖片研判，鏈路不斷</li>
  <li><strong>門控通知（二次判斷）</strong>：對誤報敏感的任務開啟後，<strong>大模型確認事件成立才發送通知，判定誤報則直接抑制</strong>——把「告警風暴」收斂成「精準提醒」，值班手機不再被誤報轟炸</li>
  <li><strong>失敗策略按現場取值</strong>：模型呼叫失敗時可選 <code>skip</code>（不改動原結果）/ <code>confirm</code>（放行通知）/ <code>reject</code>（抑制通知），在漏報與誤報之間按現場風險偏好取值，寧缺毋濫還是寧濫毋缺由你定</li>
  <li><strong>可綁定智能體與模型</strong>：研判可掛載 RAG 專家（攜帶知識庫上下文作答），也可單獨指定大模型、覆蓋研判提示詞、強制 JSON 結構化輸出，與 LLM 統一閘道一鍵切換廠商</li>
  <li><strong>結論完整回寫與展示</strong>：研判狀態（確認成立 / 誤報 / 研判中 / 研判失敗 / 未抽檢等）、信心度、理由與結構化屬性全部回寫告警；WEB 與 APP 告警列表帶狀態標籤、可按「已抽檢 / 未抽檢 / 各結論」篩選，研判詳情彈窗一屏查看圖片、結論、理由與執行資訊</li>
</ul>
...
<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
EasyAIoT 由 WEB、APP、DEVICE、EDGE、SENTINEL、VIDEO、RTC、AI、RUNTIME、POST、VISUALIZE、TWIN、TRANSFORM、PANEL、IDEA、HARNESS、SITE 等核心模組組成，並配套 COMPILE 多平臺打包交付能力：
</p>
...
<tr>
<td style="padding: 15px; border: 1px solid #e0e0e0; vertical-align: top;"><strong>SENTINEL模組</strong></td>
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>業務就緒哨兵</strong>：跟著每臺調度節點走，按勾選業務持續摸清本機是否真正可接活</li>
    <li><strong>納管即監測</strong>：節點納入平臺後自動開始巡檢，擴容當天即可驗收就緒度</li>
    <li><strong>可調度能力匯聚</strong>：把「能不能跑某類業務」匯總到管控臺，調度按真能力下發</li>
    <li><strong>缺件自助補齊</strong>：期望環境缺失時可自動請求平臺補齊，減少現場登機裝機</li>
    <li><strong>斷網友好擴容</strong>：無公網或受限網絡現場可離線納管與環境同步</li>
    <li><strong>交付口徑統一</strong>：組件健康與可調度功能同屏可見，驗收與排障少猜</li>
  </ul>
</td>
</tr>
<tr>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>NFS 媒體根</strong>：告警圖、SRS 錄像統一寫入 <strong>NFS 共享媒體根</strong>（`EASYAIOT_MEDIA_ROOT`，默認 `/mnt/easyaiot-media`；無 sudo 時 fallback 至 `$HOME/easyaiot/media`）；MQTT 只帶路徑，由 <strong>iot-sink</strong> 讀盤歸檔 RustFS</li>
  </ul>
</td>
</tr>
<tr>
...
<tr>
<td style="padding: 15px; border: 1px solid #e0e0e0; vertical-align: top;"><strong>TWIN模組</strong></td>
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;"><ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>多行業數位孪生</strong>：內建工業園區、智慧校園、智慧港口與零碳能源基地四套 Three.js 三維場景</li>
    <li><strong>三維漫遊與空間定位</strong>：支援旋轉、縮放、預設視角平滑切換，設備狀態與即時告警在場景中聯動高亮</li>
    <li><strong>樓棟樓層運維</strong>：可聚焦樓棟、切換樓層、隔離顯示本層設備，並統計在線、告警與未綁定數量</li>
    <li><strong>設備與資料綁定</strong>：編輯模式可將三維物件綁定至傳感器、攝影機、PLC / 邊緣網關，支援 MQTT、Modbus TCP、OPC UA、GB28181、ONVIF</li>
    <li><strong>可替換的模型與資料層</strong>：當前 MVP 使用程式化場景與模擬資料；穩定設備 ID 映射可後續換入 Blender / GLB 資產與 EasyAIoT API、MQTT over WebSocket 或後端 WebSocket</li>
  </ul></td>
</tr><tr>
...
| ![數位孪生](.image/banner/banner1258.png) | ![爆炸視圖](.image/banner/banner1259.png) | ![樓層視圖](.image/banner/banner1260.png) |
| ![數位孪生](.image/banner/banner1261.png) | ![爆炸視圖](.image/banner/banner1262.png) | ![樓層視圖](.image/banner/banner1263.png) |
| ![數位孪生](.image/banner/banner1264.png) | ![爆炸視圖](.image/banner/banner1265.png) | ![樓層視圖](.image/banner/banner1266.png) |
| ![數位孪生](.image/banner/banner1267.png) | ![爆炸視圖](.image/banner/banner1268.png) | ![樓層視圖](.image/banner/banner1269.png) |

#### 📹 視頻監控

| | | |
|:---:|:---:|:---:|
| ![視頻監控](.image/banner/banner1255.png) | ![視頻監控](.image/banner/banner1256.png) | ![視頻監控](.image/banner/banner1257.png) |
...
| | | |
|:---:|:---:|:---:|
| ![流程設計](.image/banner/banner1231.png) | ![節點配置](.image/banner/banner1232.png) | ![選擇審批人](.image/banner/banner1233.png) |
| ![條件分支](.image/banner/banner1234.png) | ![條件規則](.image/banner/banner1235.png) | ![並行分支](.image/banner/banner1236.png) |
...
| ![APP截圖](.image/banner/app/app_2023.png) | ![APP截圖](.image/banner/app/app_2024.png) | ![APP截圖](.image/banner/app/app_2025.png) |
| ![APP截圖](.image/banner/app/app_2026.png) | ![APP截圖](.image/banner/app/app_2027.png) | ![APP截圖](.image/banner/app/app_2028.png) |
...
| ![APP截圖](.image/banner/app/app_2029.jpg) | ![APP截圖](.image/banner/app/app_2030.jpg) | ![APP截圖](.image/banner/app/app_2031.jpg) |
...
<td style="padding: 15px; border: 1px solid #e0e0e0; color: #444; line-height: 1.8;">爲推動 EasyAIoT 項目在地圖可視化與空間研判方向的發展，獨立貢獻天地圖空間可視化能力的完整代碼實現，涵蓋國家天地圖底圖接入、攝像頭與告警設備布點、地圖分佈視圖、地點搜索與座標批量導入、告警事件自動上圖、以人/以車尋跡及移動設備軌跡回放等核心鏈路，使平臺「天地圖空間可視化與以圖研判」能力從方案設計真正走向可落地、可使用的生產形態。在此基礎上，又前瞻貢獻「沿實際道路繪製追蹤軌跡」的參考代碼——直面軌跡研判中「抓拍點直連、直線穿越樓塊水域、與真實行動路線不符」的共性痛點，提出以相鄰抓拍點爲起訖、調用地圖路線規劃服務逐段匹配可通行道路（車輛按駕車策略、人員按步行策略）再疊加渲染的技術方案，並沉澱道路規劃結果解析、失敗分段降級等關鍵實現參考，把人員與車輛的移動軌跡從「兩點連線」升級爲「沿街還原」，使尋蹤問跡的研判結論更貼近真實行動路線，顯著提升了時空研判的可讀性、可信度與實戰價值；其無私分享的可複用參考實現，也爲社區同類能力建設掃清了技術障礙、大幅降低了復現門檻。</td>
</tr>
<tr>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; color: #444; line-height: 1.8;">爲推動 EasyAIoT 項目在雲邊協同視頻接入與智能研判方向的發展，獨立貢獻多項關鍵能力，直面邊緣落地中「接不進、調不準、跑不實、查不回」的核心痛點：其一，支援邊緣攝像頭接入與算法節點親和——IPC / NVR 可選擇在線邊緣節點作爲「接入節點」，設備發現、網段掃描、ONVIF 探測與 NVR 通道枚舉全部下沉至邊緣節點就近執行，源流經邊緣節點回推主節點媒體服務統一分發播放，使分支機構、跨網段/私網中的攝像頭無需與中心平臺網路直通即可納管出流；算法任務創建時按攝像頭登記的接入節點自動鎖定執行節點，從調度機制上杜絕任務被派往無法訪問攝像頭的節點而空轉失效，讓算力真正圍繞攝像頭就近編排。其二，定位並修復同攝像頭多任務及多模型僅單路生效的核心缺陷——以任務級流標識隔離各任務的推理鏈路，透過共享解碼復用單路取流支撐多任務並發分析，並讓告警與運行狀態全鏈路攜帶任務身份，使同一攝像頭可穩定並行承載多個算法任務與多模型檢測（如人臉+車輛同框識別），且以獨立取流服務與迴歸測試長期守護該行爲。其三，支援邊緣錄影分層存儲與統一事件回放——邊緣側錄影按層級就地留存，告警事件與錄影證據可在雲邊統一入口檢索回放，補齊「事件有據可查、證據可信可回放」的證據閉環。上述工作顯著增強了平臺在真實複雜網路環境下的交付確定性與智能分析結果的可信度。</td>
</tr>
</tbody>
</table>

<p style="font-size: 14px; line-height: 1.8; color: #2c3e50; font-weight: 500; margin: 20px 0; padding: 15px; background-color: #e8f4f8; border-left: 4px solid #3498db; border-radius: 4px;">
<strong>特別緻謝</strong>：以上貢獻者在跨平臺部署文件與腳本、macOS 一鍵部署腳本奠基與鏈路探索、國標視頻能力落地與 AI 聯調驗證、多卡訓練與斷點續訓、多品牌攝像頭直連發現與批量接入、天地圖空間可視化、異構流媒體集羣部署與調度、車牌識別算法與完整代碼、EasyAIoT-Edge 邊緣側端到端串聯、校園開發者社羣組織與青年協作生態構建、物聯網設備上下行閉環與大疆司空空中視角接入、Modbus-TCP / Modbus-RTU / OPC UA 工業協議接入、攝像頭直連從發現到登入/同步/配置/多品牌出流的閉環、拖拽看板（Board）構想與 IoT 測點即時/歷史值直取集成、傳感器浮點數據預測與閾值告警規則及中心設備關聯子設備運行狀態一屏展示、AI 視頻監控分析平臺圖文介紹材料、邊緣攝像頭接入與算法節點親和調度、同攝像頭多任務及多模型僅單路生效的隔離修復、邊緣錄影分層存儲與統一事件回放等不同方面推動了 EasyAIoT 的發展，他們的專業精神與無私奉獻值得我們學習與尊敬。再次向這些傑出的貢獻者表示最誠摯的感謝！🙏
</p>
