...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>Exécuteur vidéo haute performance RUNTIME</strong> : Fait passer la vidéo de terrain de « on peut regarder / analyser » à « plus de canaux restent stables, la permanence suit » — une même capacité d’exécution couvre <strong>le jugement temps réel, les captures planifiées et la patrouille multi-canaux</strong>, tout en alimentant le <strong>mur d’images brutes multi-canaux</strong>. En pratique : d’abord des dizaines de canaux NVR clairs et fluides, puis l’IA seulement sur les canaux critiques ; basculer entre aperçu brut et jugement à cadres sans qu’un chemin n’écrase l’autre ; garder des annotations lisibles pour ne pas distraire les opérateurs. La plateforme conserve l’orchestration, l’archivage des alertes et les droits ; l’exécuteur se concentre sur le débit et la latence — augmenter canaux et scènes sans changer toute la machine. Voir <a href="RUNTIME/README.md" style="color: #3498db; text-decoration: none; font-weight: 600;">RUNTIME/README.md</a></li>
  <li><strong>Relais de flux haute performance</strong> : Conçu pour le besoin réel de « remplir d’abord le mur / l’écran partagé, sans activer l’IA sur chaque canal » — les canaux NVR et les points bâtiment créent des tâches de relais en un clic. Le mode haute performance est le défaut pour garder le multi-canal brut à l’écran à moindre coût ; les sites particuliers peuvent encore passer en mode compatibilité. Vision brute et jugement IA peuvent coexister sur le même appareil : voir le live non annoté ou basculer vers les résultats à cadres ; au démarrage/arrêt des tâches algo, la politique de relais suit automatiquement — moins de retouches manuelles. La prévisualisation côte à côte raccourcit l’acceptance en comparant latence et qualité des cadres</li>
  <li><strong>Mode atomique RUNTIME (nœuds de calcul légers)</strong> : Les boîtiers edge / workers de cluster peuvent <strong>n'installer que l'exécuteur RUNTIME</strong> — pas de VIDEO/WEB/DEVICE locaux. Agrégation alertes/heartbeats vers le VIDEO central via <code>VIDEO_BASE_URL</code> ; le centre dépêche temps réel / capture / patrouille (<code>executor=cpp</code>) sur le même binaire ; le temps réel pousse toujours par défaut le flux de détection vers le SRS central/cluster. Atomique ≠ jamais de push vidéo : atomique = pas de pile métier locale. Entrée : <code>VIDEO_BASE_URL=http://&lt;centre&gt;:6000 bash .scripts/docker/install_linux.sh runtime</code> ; ou distribution WEB / SSH iot-node</li>
  ...
  <li><strong>Gestion des tâches algorithmiques</strong> : Prend en charge la création et la gestion de tâches en temps réel, de capture et de patrouille. Chaque tâche peut être liée de manière flexible à un extracteur d'images et un trieur pour une extraction précise des images vidéo et un tri des résultats.
    <ul style="margin: 5px 0; padding-left: 20px;">
            <li><strong>Tâches algorithmiques en temps réel</strong> : Analyse live RTSP/RTMP ; backend par défaut <code>executor=cpp</code> (RUNTIME, push AI à cadres + alertes/heartbeats), <code>python</code> optionnel</li>
      <li><strong>Tâches algorithmiques de capture d'images</strong> : Analyse d'images capturées ; peut aussi utiliser <code>executor=cpp</code> (SnapScheduler / Cron)</li>
      <li><strong>Tâches algorithmiques de patrouille</strong> : Rotation multi-flux / pool ; peut aussi utiliser <code>executor=cpp</code> (PatrolScheduler)</li>
    </ul>
  </li>
  ...
  <li><strong>Redirection de flux</strong> : Regarder le live caméra sans activer l’IA ; push par lots et création auto des tâches canaux NVR prêts à l’emploi. Le mode haute performance rend le mur multi-canal plus économe — « remplir le mur d’abord, activer l’IA sur les canaux clés ensuite » ; mode compatibilité pour sites particuliers</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Orchestration vs exécution</strong> : VIDEO gère orchestration, aperçu brut, archivage d’alertes et démarrage/arrêt ; RUNTIME se concentre sur l’analyse haute capacité et l’affichage multi-canal efficace — « gouverné » et « rapide » restent séparés</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
    <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>Exécution vidéo haute performance</strong> : Accélère jugement temps réel, captures planifiées et patrouille multi-canaux, et maintient le mur multi-canal brut efficace ; la plateforme garde orchestration et archivage d’alertes, l’exécuteur se concentre sur débit et latence</li>
    ...
    <li><strong>Double vue brut + jugement</strong> : Les tâches temps réel peuvent montrer des cadres séparément de l’aperçu brut ; annotations lisibles pour moins de distraction par faux positifs</li>
    <li><strong>Mur multi-canal brut économe</strong> : Le relais haute performance est fait pour « remplir le mur d’abord », pour que « beaucoup de canaux en brut, peu en IA » coexiste sur sites NVR</li>
    <li><strong>Alertes &amp; état vers le centre</strong> : Statut d’analyse et de relais agrégé à la plateforme pour persistance et notification ; les nœuds ne stockent pas les données métier</li>
    <li><strong>Préserver le temps réel sous charge</strong> : Quand le calcul est saturé, prioriser l’image courante et la fraîcheur des alertes</li>
    <li><strong>Accélération préférée, continuité en repli</strong> : Utiliser l’accélérateur si disponible ; continuer automatiquement sinon — les tâches ne s’arrêtent pas</li>
    <li><strong>Mode atomique</strong> : Les nœuds edge peuvent n’installer que l’exécution, sans pile métier complète ; les résultats reviennent au centre pour jugement unifié</li>
    <li><strong>Distribution one-click</strong> : Console ou canaux nœud pour distribuer les exécuteurs en lot ; l’install local peut préparer le runtime automatiquement</li>
  </ul>
</td>
</tr>
<tr>
