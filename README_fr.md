...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>Couche d’exécution haute vitesse RUNTIME C++</strong> : Sort « regarder / calculer / pousser » des chaînes interprétées vers un <strong>binaire natif C++</strong> qui porte <strong>pull → décodage → inférence → callback d’alerte → push à cadres</strong>. Face aux chemins Python, il évite le GIL et le coût multi-processus — sur le même matériel : <strong>plus de canaux par boîtier, latence bout-en-bout plus basse, moins de CPU/mémoire</strong>. Temps réel / capture / patrouille et relais multi-canal brut partagent la même capacité ; défaut <code>executor=cpp</code>. En pratique : d’abord des dizaines de canaux NVR clairs, puis l’IA seulement sur les canaux critiques ; basculer aperçu brut et jugement à cadres sans qu’un chemin n’écrase l’autre ; VIDEO/WEB gardent orchestration, archivage et droits, le C++ se concentre sur débit et latence — monter en canaux sans changer toute la machine</li>
  <li><strong>Relais de flux haute performance</strong> : Conçu pour « remplir d’abord le mur / l’écran partagé, sans activer l’IA sur chaque canal » — canaux NVR et points bâtiment en un clic. Chemin par défaut : <strong>RUNTIME C++</strong> haute performance (mode compatibilité toujours possible) pour garder le multi-canal brut à moindre coût. Vision brute et jugement IA peuvent coexister ; au démarrage/arrêt des tâches algo, la politique de relais suit automatiquement. Prévisualisation côte à côte pour comparer latence et cadres à l’acceptance</li>
  <li><strong>Mode atomique : le boîtier n’installe que l’exécuteur C++</strong> : Les boîtiers edge / workers peuvent <strong>n’installer que le binaire RUNTIME</strong> — pas de VIDEO / WEB / DEVICE locaux — install plus léger, surface de panne plus étroite, extension comme « ajouter du calcul » plutôt que « réinstaller une plateforme ». Les nœuds agrègent alertes et heartbeats vers le VIDEO central ; le centre dépêche temps réel / capture / patrouille (<code>executor=cpp</code>) sur le même processus C++ ; le temps réel pousse toujours par défaut le flux à cadres vers le SRS central. Atomique = pas de pile métier locale, pas « jamais de push vidéo » ; intelligence et images convergent au centre. Une commande : <code>VIDEO_BASE_URL=http://&lt;centre&gt;:6000 bash .scripts/docker/install_linux.sh runtime</code> ; ou distribution WEB par lots</li>
  ...
  <li><strong>Analyse IA en temps réel des flux vidéo des caméras</strong> : Pour les flux RTSP/RTMP, pipeline complet pull/décodage → inférence → sortie structurée → alertes ; par défaut via <strong>RUNTIME C++</strong> (même exécuteur que capture/patrouille), réponse en millisecondes, et <strong>push par défaut du flux de détection à cadres</strong> pour bascule opérateur. Vision (brut) et algorithme (détection) indépendants ; résultats liés aux zones, périodes, visage/plaque et post-traitement orchestrable</li>
  ...
  <li><strong>Gestion des tâches algorithmiques</strong> : Prend en charge la création et la gestion de tâches en temps réel, de capture et de patrouille. Chaque tâche peut être liée de manière flexible à un extracteur d'images et un trieur pour une extraction précise des images vidéo et un tri des résultats.
    <ul style="margin: 5px 0; padding-left: 20px;">
            <li><strong>Tâches algorithmiques en temps réel</strong> : Analyse live RTSP/RTMP ; backend par défaut <code>executor=cpp</code> (démarre <strong>RUNTIME C++</strong>, push AI à cadres + alertes/heartbeats), chemin <code>python</code> de compatibilité optionnel</li>
      <li><strong>Tâches algorithmiques de capture d'images</strong> : Analyse d'images capturées ; peut aussi utiliser <code>executor=cpp</code> (C++ SnapScheduler / Cron)</li>
      <li><strong>Tâches algorithmiques de patrouille</strong> : Rotation multi-flux / pool ; peut aussi utiliser <code>executor=cpp</code> (C++ PatrolScheduler) ; les nœuds peuvent accueillir en mode atomique (exécuteur seul)</li>
    </ul>
  </li>
  ...
  <li><strong>Redirection de flux</strong> : Regarder le live caméra sans activer l’IA ; push par lots et création auto des tâches canaux NVR prêts à l’emploi. Chemin par défaut <strong>RUNTIME C++</strong> haute performance — « remplir le mur d’abord, IA sur les canaux clés » ; sites particuliers : mode compatibilité Python/FFmpeg</li>
...
<p style="font-size: 14px; line-height: 1.8; color: #555; margin: 15px 0;">
Java excelle pour construire une architecture de plateforme stable et fiable, mais convient mal à la programmation réseau et à l'IA ; Python excelle en programmation réseau et en algorithmes d'IA, mais bute sur le GIL et le coût multi-processus dès qu’il s’agit d’exécution frame haute densité / basse latence ; C++ excelle pour l'exécution haute performance, mais convient mal à l'architecture de plateforme et à l’orchestration algorithmique — la plateforme fait donc descendre <strong>pull/décodage, inférence, push à cadres et relais multi-canal brut</strong> dans <strong>RUNTIME C++</strong>, tandis que WEB/VIDEO gardent orchestration et surface métier ; les boîtiers edge peuvent aussi utiliser le <strong>mode atomique</strong> et n’installer que ce binaire C++, avec jugement unifié au centre. Go excelle en réseau à haute concurrence et en implémentation de protocoles, mais convient mal au plan de contrôle plateforme et aux algorithmes d'IA ; TypeScript excelle pour des interactions front-end complexes et des interfaces industrialisées à typage sûr, mais convient mal au calcul backend haute performance et à l'inférence IA. EasyAIoT adopte une architecture hybride cinq-en-un, où chaque langage occupe son terrain de jeu — une plateforme AIoT ambitieuse à réaliser, mais extrêmement simple à utiliser.
</p>
...
<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
EasyAIoT comprend notamment WEB, APP, DEVICE, NODE, VIDEO, RTC, AI, RUNTIME, EDGE, VISUALIZE, TRANSFORM, PANEL et SITE, avec COMPILE pour le packaging et la livraison multiplateformes :
</p>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Rôles de nœud</strong> : Rôles compute / media / hybrid pour l'ordonnancement inter-nœuds et l'échelle élastique ; les nœuds de calcul peuvent utiliser le <strong>mode atomique RUNTIME</strong> (exécuteur C++ seul) pour accueillir temps réel / capture / patrouille et relais haute performance dépêchés par le centre</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Orchestration vs exécution</strong> : VIDEO gère orchestration, aperçu brut, archivage d’alertes et démarrage/arrêt ; le lourd travail va à <strong>RUNTIME C++</strong> (inférence et relais haute performance) — « gouverné » et « rapide » séparés, pour que canaux et latence ne soient pas freinés par l’orchestration Python</li>
...
<tr>
<td style="padding: 15px; border: 1px solid #e0e0e0; vertical-align: top;"><strong>Module RUNTIME (C++)</strong></td>
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
    <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>Exécution native C++</strong> : Un binaire porte pull, décodage, inférence ONNX, callbacks alerte/heartbeat et push à cadres ; défaut <code>executor=cpp</code> — plus de canaux par boîtier, latence plus basse, ressources plus légères que les chemins Python</li>
    ...
    <li><strong>Mur multi-canal brut économe</strong> : Le relais utilise par défaut le chemin C++ haute performance pour que « beaucoup de canaux en brut, peu d’IA » coexiste sur sites NVR</li>
    ...
    <li><strong>Mode atomique</strong> : Boîtiers / workers n’installent que l’exécuteur C++ — pas de VIDEO/WEB/DEVICE ; le centre orchestre, le nœud exécute — étendre comme ajouter du calcul, pas une plateforme complète</li>
...
<p style="font-size: 14px; line-height: 1.8; color: #555; margin: 10px 0;">
La plateforme prend en charge les tâches algorithmiques en temps réel, de capture et de patrouille : les trois peuvent utiliser par défaut la <strong>couche d’exécution haute vitesse RUNTIME C++</strong> (<code>executor=cpp</code>) — binaire natif pour pull longue durée, décodage, inférence YOLO et callbacks ; temps réel avec <strong>push de flux AI à cadres par défaut</strong> ; capture Cron ; patrouille multi-flux ; le relais peut aussi emprunter le même chemin C++ pour un mur brut économe. Face au backend Python de compatibilité, le C++ tient mieux la densité de canaux et la basse latence. Alertes et heartbeats reviennent à VIDEO ; les nœuds de calcul peuvent activer le <strong>mode atomique</strong> — exécuteur C++ seul, surface métier au centre — boîtiers légers et extension par ajout de nœuds. La gestion des tâches garde extraction/tri flexibles ; l’inférence cluster des services de modèles assure réponse millisecondes et haute disponibilité. Deux stratégies de surveillance (complète / partielle) permettent des règles précises par périodes.
</p>
<p style="font-size: 14px; line-height: 1.8; color: #555; margin: 10px 0;">
