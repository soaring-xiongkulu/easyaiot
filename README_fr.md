...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>Exécuteur de frames haute vitesse RUNTIME C++</strong> : Fait évoluer l'ancien TASK en module autonome <code>RUNTIME</code>, dédié au chemin chaud vidéo temps réel — pull, décodage matériel/logiciel, inférence YOLO et émission de résultats structurés. Les étapes sont découplées par des files annulaires (éviction du plus ancien si plein) avec latence bout-en-bout et pertes de frames observables ; VIDEO le démarre par tâche via <code>executor=cpp</code>, tandis que alertes et heartbeats empruntent toujours le hook / Kafka VIDEO, et que le relais d'aperçu et l'orchestration restent dans VIDEO — <strong>sans remplacer ni court-circuiter la couche plateforme vidéo</strong>. Par défaut <code>executor=python</code> pour la compatibilité production ; basculez en un clic vers le backend C++ pour un débit maximal, rendant « orchestration côté VIDEO, calcul côté RUNTIME » un chemin d'accélération hybride concrètement déployable</li>
  ...
  <li><strong>Analyse IA en temps réel des flux vidéo des caméras</strong> : Pour les flux vidéo RTSP/RTMP en temps réel, construit un pipeline d'analyse complet « tirage et décodage → extraction intelligente d'images → inférence de modèle → sortie structurée → liaison d'alertes », transformant instantanément les changements de scène en événements de détection structurés consultables et analysables avec une réponse en millisecondes. La chaîne de visionnage et la chaîne algorithmique sont architecturalement découplées, conciliant clarté de prévisualisation et débit concurrentiel élevé. Les résultats d'analyse s'intègrent sans friction aux régions de détection, périodes de défense, reconnaissance faciale/plaques et règles de post-traitement orchestrables, faisant évoluer le modèle de surveillance traditionnel « humain devant l'écran, revue a posteriori » vers « machine en veille permanente, anomalies poussées en secondes, preuves archivées automatiquement », transformant la vidéo en temps réel d'une visualisation passive en infrastructure de perception active et d'analyse intelligente</li>
  ...
  <li><strong>Gestion des tâches algorithmiques</strong> : Prend en charge la création et la gestion de tâches en temps réel, de capture et de patrouille. Chaque tâche peut être liée de manière flexible à un extracteur d'images et un trieur pour une extraction précise des images vidéo et un tri des résultats.
    <ul style="margin: 5px 0; padding-left: 20px;">
      <li><strong>Tâches algorithmiques en temps réel</strong> : Analyse en temps réel des flux RTSP/RTMP ; backend d'exécution <code>python</code> (par défaut, run_deploy existant) ou <code>cpp</code> (démarre le binaire RUNTIME). Adaptées à la surveillance, la sécurité et autres scénarios temps réel.</li>
      <li><strong>Tâches algorithmiques de capture d'images</strong> : Analyse intelligente des images capturées. Adaptées à la revue d'événements, la recherche d'images, etc.</li>
      <li><strong>Tâches algorithmiques de patrouille</strong> : Rotation multi-flux et pool de connexions — « moins de connexions, plus de couverture » pour la patrouille IA par lots.</li>
    </ul>
  </li>
  ...
  <li><strong>Séparation des chaînes visualisation / analyse et débits hiérarchisés</strong> : La prévisualisation et les murs vidéo sont découplées de l'extraction d'images pour l'analyse algorithmique, avec planification indépendante. La voie visualisation privilégie une image nette et fluide ; la voie analyse équilibre précision de détection, calcul et bande passante, évitant que l'analyse et la visualisation se disputent un même canal — pour concilier « image claire et stable » et analyse évolutive.</li>
...
<p style="font-size: 14px; line-height: 1.8; color: #555; margin: 15px 0;">
EasyAIoT n'est pas vraiment un seul projet, mais plusieurs sous-projets déployables indépendamment (WEB, DEVICE, VIDEO, AI, etc.).
</p>
...
<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
EasyAIoT comprend notamment WEB, DEVICE, NODE, VIDEO, AI, RUNTIME, EDGE, VISUALIZE, TRANSFORM, PANEL, avec COMPILE pour le packaging et la livraison multiplateformes :
</p>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Gestion des tâches algorithmiques</strong> : Prend en charge les tâches temps réel / capture / patrouille ; les tâches temps réel peuvent configurer le backend <code>executor=python|cpp</code> (python par défaut ; en cpp, ce module génère l'ini et démarre RUNTIME).</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>Chemin chaud frames C++</strong> : Évolution de l'ancien TASK ; concentré sur pull → décodage → inférence YOLO → émission des résultats pour les tâches temps réel VIDEO <code>executor=cpp</code> (ne remplace pas l'orchestration / aperçu / alertes VIDEO)</li>
    <li><strong>Pipeline files annulaires</strong> : Threads Pull/Decode, Infer, Emit par étapes ; l'anneau de frames évince le plus ancien si plein, priorité temps réel et décodage non bloqué par l'inférence</li>
    <li><strong>Contrat VIDEO aligné</strong> : Alertes POST <code>/video/alert/hook</code>, heartbeats POST <code>/video/algorithm/heartbeat/realtime</code>, <code>/health</code> expose pertes de frames et latence</li>
    <li><strong>Friendly edge Linux</strong> : CMake produit un binaire autonome déployable avec conda / dépendances système ; l'exécuteur python par défaut reste un repli immédiat</li>
  </ul>
</td>
</tr>
<tr>
