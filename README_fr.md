...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>Mode atomique : le boîtier n’installe que l’exécuteur</strong> : Les boîtiers edge / workers peuvent <strong>n’installer que le binaire RUNTIME</strong> — pas de VIDEO / WEB / DEVICE locaux — install plus léger, surface de panne plus étroite, extension comme « ajouter du calcul » plutôt que « réinstaller une plateforme ». Les nœuds agrègent alertes et heartbeats vers le VIDEO central ; le centre dépêche temps réel / capture / patrouille (<code>executor=cpp</code>) sur le même processus d’exécution ; le temps réel pousse toujours par défaut le flux à cadres vers le SRS central. Atomique = pas de pile métier locale, pas « jamais de push vidéo » ; intelligence et images convergent au centre. Une commande : <code>VIDEO_BASE_URL=http://&lt;centre&gt;:6000 bash .scripts/docker/install_linux.sh runtime</code> ; ou distribution WEB par lots</li>
  ...
  <li><strong>Calcul edge atomique RUNTIME</strong> : Pour déploiement à large couverture et extension de boîtiers de calcul — les nœuds n'installent que l'exécuteur RUNTIME ; alertes via MQTT vers iot-sink, heartbeats vers VIDEO ; inutile d'empiler toute la pile métier sur chaque site. L'ancienne suite EDGE fédérée a été retirée ; chemin unifié</li>
  ...
  <li><strong>Gestion des tâches algorithmiques</strong> : Prend en charge la création et la gestion de tâches en temps réel, de capture et de patrouille. Chaque tâche peut être liée de manière flexible à un extracteur d'images et un trieur pour une extraction précise des images vidéo et un tri des résultats.
    <ul style="margin: 5px 0; padding-left: 20px;">
      ...
      <li><strong>Tâches algorithmiques de patrouille</strong> : Rotation multi-flux / pool ; peut aussi utiliser <code>executor=cpp</code> (RUNTIME PatrolScheduler) ; les nœuds peuvent accueillir en mode atomique (exécuteur seul)</li>
    </ul>
  </li>
...
<p style="font-size: 14px; line-height: 1.8; color: #555; margin: 15px 0;">
Java excelle pour construire une architecture de plateforme stable et fiable, mais convient mal à la programmation réseau et à l'IA ; Python excelle en programmation réseau et en algorithmes d'IA, mais bute sur le GIL et le coût multi-processus dès qu’il s’agit d’exécution frame haute densité / basse latence ; C++ excelle pour l'exécution haute performance, mais convient mal à l'architecture de plateforme et à l’orchestration algorithmique — la plateforme fait donc descendre <strong>pull/décodage, inférence, push à cadres et relais multi-canal brut</strong> dans <strong>RUNTIME</strong>, tandis que WEB/VIDEO gardent orchestration et surface métier ; les boîtiers edge peuvent aussi utiliser le <strong>mode atomique</strong> et n’installer que cet exécuteur, avec jugement unifié au centre. Go excelle en réseau à haute concurrence et en implémentation de protocoles, mais convient mal au plan de contrôle plateforme et aux algorithmes d'IA ; TypeScript excelle pour des interactions front-end complexes et des interfaces industrialisées à typage sûr, mais convient mal au calcul backend haute performance et à l'inférence IA. EasyAIoT adopte une architecture hybride cinq-en-un, où chaque langage occupe son terrain de jeu — une plateforme AIoT ambitieuse à réaliser, mais extrêmement simple à utiliser.
</p>
...
<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
EasyAIoT comprend notamment WEB, APP, DEVICE, NODE, VIDEO, RTC, AI, RUNTIME, VISUALIZE, TRANSFORM, PANEL et SITE, avec COMPILE pour le packaging et la livraison multiplateformes (EDGE retiré ; edge via mode atomique RUNTIME) :
</p>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Rôles de nœud</strong> : Rôles compute / media / hybrid pour l'ordonnancement inter-nœuds et l'échelle élastique ; les nœuds de calcul peuvent utiliser le <strong>mode atomique RUNTIME</strong> (exécuteur seul) pour accueillir temps réel / capture / patrouille et relais haute performance dépêchés par le centre</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
    ...
    <li><strong>Mode atomique</strong> : Boîtiers / workers n’installent que l’exécuteur — pas de VIDEO/WEB/DEVICE ; le centre orchestre, le nœud exécute — étendre comme ajouter du calcul, pas une plateforme complète</li>
...
<tr>
<td style="padding: 15px; border: 1px solid #e0e0e0; vertical-align: top;"><strong>Edge atomique RUNTIME</strong></td>
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>Module EDGE retiré</strong> : Calcul edge unifié via le <strong>mode atomique RUNTIME</strong> (exécuteur seul)</li>
    <li><strong>Plan événements MQTT</strong> : Alertes/captures/post-traitement via bus algo EMQX ; <strong>iot-sink</strong> persiste, archive et enrichit les notifications</li>
    <li><strong>Plan gestion HTTP heartbeat</strong> : Vivacité des tâches vers VIDEO ; démarrage/arrêt et table des tâches gérés par VIDEO</li>
    <li><strong>Racine média NFS</strong> : Images d'alerte et DVR SRS unifiés sur la <strong>racine média NFS partagée</strong> (<code>EASYAIOT_MEDIA_ROOT</code>, défaut <code>/mnt/easyaiot-media</code> ; fallback <code>$HOME/easyaiot/media</code> sans sudo) ; MQTT ne transporte que les chemins ; <strong>iot-sink</strong> lit le disque et archive vers MinIO</li>
  </ul>
</td>
</tr>
<tr>
