...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  <li><strong>Exécuteur de frames haute vitesse RUNTIME C++</strong> : Fait passer le chemin chaud vidéo/algorithme de « ça tourne » à « ça monte en charge » — un binaire <code>RUNTIME</code> unique couvre <strong>trois</strong> types de tâches : <strong>temps réel</strong> (pull longue durée + Pipeline), <strong>capture</strong> (Cron SnapScheduler), <strong>patrouille</strong> (PatrolScheduler multi-appareils). Chemin commun : <strong>pull/capture → décodage → inférence YOLO → alertes/heartbeats vers VIDEO</strong> ; files annulaires (éviction du plus ancien) avec latence et pertes observables. VIDEO le démarre via <code>executor=cpp</code> ; orchestration, aperçu brut et persistance d'alertes restent dans VIDEO — <strong>sans remplacer la couche plateforme vidéo</strong>. Les tâches <strong>temps réel</strong> formelles <strong>poussent par défaut un flux AI indépendant avec cadres</strong> (SRS <code>ai/{device}</code>), séparé de l'aperçu <code>live/</code> ; capture/patrouille privilégient résultats structurés et alertes (push possible si URL, sans push long forcé). Prefer GPU avec repli CPU. Voir <a href="RUNTIME/README.md" style="color: #3498db; text-decoration: none; font-weight: 600;">RUNTIME/README.md</a></li>
  ...
  <li><strong>Séparation des chaînes visualisation / analyse et débits hiérarchisés</strong> : Aperçu / mur d'images découplés de l'analyse. La vision utilise SRS <code>live/</code> (flux brut) ; l'algorithme est porté par RUNTIME (ou Python) pour temps réel / capture / patrouille ; les tâches temps réel poussent par défaut le flux à cadres vers <code>ai/</code></li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>Gestion des tâches algorithmiques</strong> : Tâches temps réel / capture / patrouille ; les trois peuvent utiliser <code>executor=cpp</code> par défaut (ini + RUNTIME) ; le temps réel pousse aussi <code>ai/</code> par défaut ; <code>python</code> optionnel.</li>
    <li><strong>Répartition avec RUNTIME</strong> : VIDEO orchestre, aperçu brut (<code>live/</code>) et persistance d'alertes ; RUNTIME exécute temps réel / capture / patrouille côté frames, avec push à cadres (<code>ai/</code>) en temps réel</li>
    <li><strong>Surface de résultats unifiée</strong> : Que l'exécuteur soit Python ou C++, les alertes reviennent vers <code>/video/alert/hook</code> de ce module, les heartbeats mettent à jour l'état du service de tâches, puis Kafka / persistance / notifications.</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
    <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>Chemin chaud frames C++</strong> : Un binaire pour <strong>realtime / snap / patrol</strong> — pull ou capture planifiée → décodage → YOLO → émission ; sert VIDEO <code>executor=cpp</code></li>
    <li><strong>Trois formes de tâches</strong> : Pipeline temps réel ; Cron snap ; rotation multi-appareils</li>
    <li><strong>Push AI temps réel par défaut</strong> : SRS <code>ai/{device}</code> séparé de <code>live/</code></li>
    <li><strong>Agrégation alertes &amp; heartbeats</strong> : Hook / heartbeat VIDEO → Kafka / persistance</li>
    <li><strong>Pipeline files annulaires</strong> : drop oldest si plein</li>
    <li><strong>GPU préféré, repli CPU</strong></li>
    <li><strong>Mode atomique</strong> : RUNTIME seul sur nœuds de calcul ; <code>VIDEO_BASE_URL</code> vers le centre</li>
    <li><strong>Distribution one-click</strong> : WEB / iot-node → <code>/opt/easyaiot/RUNTIME</code></li>
  </ul>
</td>
</tr>
<tr>
