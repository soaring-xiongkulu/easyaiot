...
📦 <strong>安装包下载</strong>：Ubuntu / Debian、CentOS / RHEL、<strong>欧拉(openEuler) 24.x</strong>、Windows、macOS 及 ARM / 麒麟(Kylin)等目标安装包见 <a href="https://gitee.com/volara/easyaiot/releases" style="color: #3498db; text-decoration: none; font-weight: 600;">Gitee Releases</a>。
</p>
...
<p style="font-size: 15px; line-height: 1.8; color: #333; margin: 15px 0;">
EasyAIoT积极响应本土化战略，全面支持本土化硬件和操作系统，为用户提供安全可控的AIoT解决方案。新增 <strong>欧拉(openEuler) 24.x</strong> 一键部署与 PANEL RPM 打包：入口脚本自动卸载自带旧版 docker-engine、修复 Docker CE 仓库版本号、配置国内镜像源与 firewalld，再完成平台全量/分档安装；COMPILE 可产出 欧拉(openEuler) 专用安装包，配合 <code>install_linux_openeuler.sh</code> 实现「到场可装、值守可守」。
</p>
...
<ul style="font-size: 14px; line-height: 1.8; margin: 10px 0; padding-left: 20px;">
  <li>兼容<strong>欧拉(openEuler) 24.x</strong>（一键部署脚本 + PANEL RPM 安装包）</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>多平台交付物</strong>：把 PANEL 等能力打成 Ubuntu / Debian、CentOS / RHEL、<strong>欧拉(openEuler) 24.x</strong>、Windows、macOS 及 ARM / 麒麟(Kylin)等目标安装包或可执行文件，方便给客户装机，不必现场从源码编译</li>
...
<ul style="font-size: 14px; line-height: 1.8; margin: 10px 0; padding-left: 20px;">
  ...
  <li>覆盖 Ubuntu、CentOS/RHEL、<strong>欧拉(openEuler) 24.x</strong>、银河麒麟(Kylin)等主流服务器发行版</li>
</ul>
</div>
...
- [平台部署文档](.doc/部署文档/平台部署文档_zh.md) — Linux（含 Ubuntu / CentOS / **欧拉(openEuler) 24.x** / ARM / 麒麟(Kylin)）/ Mac / Windows 分步部署指南
...
- [部署最佳实践](.doc/部署文档/部署最佳实践.md) — 规格选型、环境要求、一键部署（含 欧拉(openEuler)）、预构建镜像、运维排障与生产环境建议
