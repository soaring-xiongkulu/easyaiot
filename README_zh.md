...
📄 如需更完整的图文介绍，请参阅 <a href=".doc/项目介绍/EasyAIoT项目介绍 V2.0.pptx" style="color: #3498db; text-decoration: none; font-weight: 600;">EasyAIoT项目介绍 V2.0（PPT）</a>，以及 <a href=".doc/项目介绍/AI视频监控分析平台.pdf" style="color: #3498db; text-decoration: none; font-weight: 600;">AI视频监控分析平台（PDF）</a>。
</p>
...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>RUNTIME C++ 高速帧执行器</strong>：把视频热路径从「能跑」推进到「跑得动、扩得开」——独立 <code>RUNTIME</code> 模块以<strong>同一二进制</strong>承接三类算法任务，并覆盖<strong>原画推流转发</strong>：<strong>实时</strong>（长连接拉流 + Pipeline）、<strong>抓拍</strong>（Cron 调度 SnapScheduler）、<strong>巡检</strong>（多设备轮巡 PatrolScheduler）、<strong>forward-only 原画直通</strong>（RTSP→RTMP codec copy，无 YOLO/重编码）。算法共性路径为<strong>拉流/采帧 → 解码 → YOLO 推理 → 告警/心跳回传</strong>；流水线以环形队列解耦（满则丢最旧帧），端到端时延与丢帧可观测。VIDEO 以 <code>executor=cpp</code> 按任务拉起，编排、原画预览与告警落库仍在 VIDEO，<strong>不替代视频平台层</strong>。其中<strong>正式实时任务默认额外推独立 AI 检测流</strong>（SRS <code>ai/{设备}</code>），与推流转发原画 <code>live/</code> 双路分离——值守既能看清原画，又能切到带框图研判；叠框样式与 VIDEO Python 路径对齐（绿色细框、框上类别名），按分辨率自适应线宽，降低「框又粗又糊」带来的研判干扰。抓拍/巡检以结构化结果与告警为主（有地址时亦可推流，但不强制长推）。默认 prefer GPU（失败自动回退 CPU）。需要极致吞吐时一键走 C++ 后端，让「编排归 VIDEO、算力与转发热路径归 RUNTIME」成为可落地的混编加速路径。详见 <a href="RUNTIME/README.md" style="color: #3498db; text-decoration: none; font-weight: 600;">RUNTIME/README.md</a></li>
  <li><strong>高性能推流转发（RUNTIME forward）</strong>：NVR 数十路通道、园区多点位「只要原画、不要每路都开 AI」时，传统 FFmpeg 逐路转码容易把 CPU 打满、延时抬高。推流转发任务默认 <code>executor=cpp</code>，由 RUNTIME <code>task_type=forward</code> 做<strong>编码透传（codec copy）</strong>——一路摄像头约几个百分点 CPU，即可把原画稳推到 SRS <code>live/</code>；需要兼容老环境时仍可选 <code>python</code>（FFmpeg）路径。WEB 创建/编辑任务可切换「高性能 / 兼容模式」，NVR 自动建任务与单摄像头自动建任务默认走高性能。与算法任务可同设备共存：原画走 <code>live/</code>、AI 带框走 <code>ai/</code>，互不抢同一推流地址；算法启停后自动刷新关联转发配置，避免策略变更后仍用旧拉流参数。配套 <code>VIDEO/tools/runtime_stream_viewer</code> 可一键并排对比原画与 AI 延时、叠框效果，缩短联调与验收时间</li>
  ...
  <li><strong>推流转发</strong>：支持在无需启用 AI 的情况下直接观看摄像头实时画面；多路批量推送、NVR 通道自动建任务均可开箱使用。默认走 <strong>RUNTIME forward（codec copy）</strong> 高性能路径，CPU 占用远低于逐路重编码，适合「多路原画上墙、只对少数通道开 AI」的园区与楼宇场景；亦可切回 FFmpeg 兼容模式以适配特殊编码环境</li>
  ...
  <li><strong>观看链路与算法链路分离及分级码率</strong>：将「实时预览/大屏观看」与「算法分析」相互解耦、独立调度。观看侧由推流转发（默认可 RUNTIME forward）推到 SRS <code>live/</code> 原画，优先保障画清晰、少卡顿；算法侧由 RUNTIME（或 Python 执行器）承接实时 / 抓拍 / 巡检，正式实时任务默认把带框检测流推到独立 <code>ai/</code> 应用，避免分析与观看争抢同一通道与同一推流地址，保障「看得清、不卡断」与「算得动、可扩展」兼顾</li>
...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>推流转发</strong>：随时创建与启停转发任务，掌握集群节点与各路流状态；默认可走 RUNTIME 高性能原画直通，远程也能调度视频资源</li>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    ...
    <li><strong>算法任务管理</strong>：支持实时 / 抓拍 / 巡检算法任务；三类任务均可默认 <code>executor=cpp</code>（生成 ini 并拉起 RUNTIME），实时任务另默认推 <code>ai/</code> 检测流；亦可选 <code>python</code></li>
    <li><strong>推流转发编排</strong>：任务默认 <code>executor=cpp</code>，生成 forward ini 并按设备监督 RUNTIME 直通进程；心跳直连 VIDEO，避免网关未就绪时 worker 误退；亦可选 FFmpeg 兼容路径</li>
    <li><strong>抽帧器与排序器</strong>：支持灵活的抽帧策略与结果排序机制，每个算法任务可绑定独立的抽帧器和排序器</li>
    <li><strong>布防时段</strong>：支持全防模式和半防模式的时段化配置</li>
    <li><strong>与 RUNTIME 分工</strong>：VIDEO 负责编排、原画预览（<code>live/</code>）、告警落库与任务启停；RUNTIME 统一承接实时 / 抓拍 / 巡检的帧侧执行，以及 forward-only 原画直通；实时任务额外推带框检测流（<code>ai/</code>）并 hook 回传</li>
    <li><strong>联调预览工具</strong>：<code>tools/runtime_stream_viewer</code> 并排对比 <code>live/</code> 与 <code>ai/</code>，便于验收延时与叠框效果</li>
  </ul>
</td>
</tr>
<tr>
...
<td style="padding: 15px; border: 1px solid #e0e0e0; line-height: 1.8; color: #444;">
  <ul style="margin: 5px 0; padding-left: 20px;">
    <li><strong>C++ 帧热路径</strong>：由原 TASK 演进的独立执行器，以同一二进制承接 <strong>realtime / snap / patrol / forward</strong>——算法路径为拉流或调度采帧 → 解码 → YOLO 推理 → 结构化结果回传；转发路径为 RTSP→RTMP codec copy（无推理/重编码）；服务 VIDEO <code>executor=cpp</code>，不替代 VIDEO 编排 / 预览 / 告警面</li>
    <li><strong>四类任务形态</strong>：实时长连接 Pipeline；抓拍 Cron（SnapScheduler）；巡检多设备轮巡（PatrolScheduler）；原画直通 StreamForwarder——调度策略不同，心跳/告警契约仍回 VIDEO</li>
    <li><strong>实时默认推带框检测流</strong>：正式 realtime 任务启动即推独立 SRS <code>ai/{设备}</code>，与推流转发原画 <code>live/</code> 双路分离；叠框对齐 VIDEO 绿色细框样式，并应用任务置信度阈值，减少低分误检干扰研判；抓拍/巡检以结果与告警为主（有地址时可推，不强制长推）</li>
    <li><strong>forward-only 原画直通</strong>：推流转发高性能路径专用；低延迟 RTSP 选项 + copy 推流，NVR 多路场景下 CPU 占用显著低于 FFmpeg 重编码，让「多路看原画、少路跑 AI」可同时成立</li>
    <li><strong>告警与心跳汇聚</strong>：HTTP 回传 VIDEO alert hook / heartbeat（realtime、snap 走 realtime 心跳口；patrol 走巡检心跳口；forward 走推流转发心跳口），经 Kafka 落库与通知，节点本地不落业务库</li>
    <li><strong>环形队列流水线</strong>：拉流/解码、推理、回传分阶段线程；帧环满则丢最旧，保障实时优先、解码不被推理拖死</li>
    <li><strong>GPU 优先、失败回退</strong>：默认 prefer CUDA EP，失败自动 CPU，任务不中断</li>
    <li><strong>原子模式</strong>：计算节点只装 RUNTIME（不装 VIDEO/WEB），<code>VIDEO_BASE_URL</code> 汇聚中心；正式任务（实时/抓拍/巡检）下发后由同一二进制执行，实时任务仍可推检测流到中心媒体面——节点轻、中心仍看得见、管得住</li>
    <li><strong>一键分发</strong>：WEB「业务运行时分发」或 iot-node SSH 批量安装到 <code>/opt/easyaiot/RUNTIME</code>；本机 VIDEO 安装入口经 <code>ensure_runtime_cpp.sh</code> 自动编译挂载</li>
  </ul>
</td>
</tr>
<tr>
