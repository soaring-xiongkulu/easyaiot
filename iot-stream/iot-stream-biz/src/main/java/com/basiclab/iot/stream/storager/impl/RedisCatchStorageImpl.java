package com.basiclab.iot.stream.storager.impl;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import com.basiclab.iot.stream.common.ServerInfo;
import com.basiclab.iot.stream.common.SystemAllInfo;
...
import com.basiclab.iot.stream.mapper.DeviceMapper;
...
import com.basiclab.iot.stream.service.bean.MessageForPushChannel;
import com.basiclab.iot.stream.storager.IRedisCatchStorage;
import com.basiclab.iot.stream.utils.DateUtil;
import com.basiclab.iot.stream.utils.JsonUtil;
import com.basiclab.iot.stream.utils.SystemInfoUtils;
import com.basiclab.iot.stream.utils.redis.RedisUtil;
...
public class RedisCatchStorageImpl implements IRedisCatchStorage {
    ...
    public void updateWVPInfo(ServerInfo serverInfo, int time) {
        String key = VideoManagerConstants.LF_SERVER_PREFIX + userSetting.getServerId();
        Duration duration = Duration.ofSeconds(time);
        redisTemplate.opsForValue().set(key, serverInfo, duration);
        // 设置平台的分数值
        String setKey = VideoManagerConstants.LF_SERVER_LIST;
    ...
    public void sendStreamChangeMsg(String type, JSONObject jsonObject) {
        String key = VideoManagerConstants.LF_MSG_STREAM_CHANGE_PREFIX + type;
    ...
    public void addStream(MediaServer mediaServerItem, String type, String app, String streamId, MediaInfo mediaInfo) {
        ...
        String key = VideoManagerConstants.LF_SERVER_STREAM_PREFIX  + userSetting.getServerId() + "_" + type.toUpperCase() + "_" + app + "_" + streamId + "_" + mediaServerItem.getId();
    ...
    public void removeStream(String mediaServerId, String type, String app, String streamId) {
        String key = VideoManagerConstants.LF_SERVER_STREAM_PREFIX + userSetting.getServerId() + "_" + type.toUpperCase() + "_"  + app + "_" + streamId + "_" + mediaServerId;
        redisTemplate.delete(key);
    }

    @Override
    public void removeStream(String mediaServerId, String type) {
        String key = VideoManagerConstants.LF_SERVER_STREAM_PREFIX + userSetting.getServerId() + "_" + type.toUpperCase() + "_*_*_" + mediaServerId;
    ...
    public void updateGpsMsgInfo(GPSMsgInfo gpsMsgInfo) {
        String key = VideoManagerConstants.LF_STREAM_GPS_MSG_PREFIX + userSetting.getServerId();
    ...
    public MediaInfo getStreamInfo(String app, String streamId, String mediaServerId) {
        String scanKey = VideoManagerConstants.LF_SERVER_STREAM_PREFIX  + userSetting.getServerId() + "_*_" + app + "_" + streamId + "_" + mediaServerId;
    ...
    public MediaInfo getProxyStream(String app, String streamId) {
        String scanKey = VideoManagerConstants.LF_SERVER_STREAM_PREFIX  + userSetting.getServerId() + "_PULL_" + app + "_" + streamId + "_*";
    ...
    public int getPushStreamCount(String id) {
        String key = VideoManagerConstants.LF_SERVER_STREAM_PREFIX + userSetting.getServerId() + "_PUSH_*_*_" + id;
        return RedisUtil.scan(redisTemplate, key).size();
    }

    @Override
    public int getProxyStreamCount(String id) {
        String key = VideoManagerConstants.LF_SERVER_STREAM_PREFIX + userSetting.getServerId() + "_PULL_*_*_" + id;
        return RedisUtil.scan(redisTemplate, key).size();
    }
    ...
    public ServerInfo queryServerInfo(String serverId) {
        String key = VideoManagerConstants.LF_SERVER_PREFIX + serverId;
        return (ServerInfo)redisTemplate.opsForValue().get(key);
    }

    @Override
    public String chooseOneServer(String serverId) {
        String key = VideoManagerConstants.LF_SERVER_LIST;
